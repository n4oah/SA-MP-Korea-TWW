...
//define
#define HOST_NAME 		"&The World War Role Play [forum.tww-rp.kro.kr]"
#define MAP_NAME		"오픈베타"
#define MODE_NAME		"TWW:RP 0.04v"
#define FORUM			"forum.tww-rp.kro.kr"

#define SERVER_NAME      "The World War Role Playing Game"
#define SCRIPT_VERSION 	"TWWRP 0.04v"
#define MANAGER      	"Master"
#define MODEMAKER      	"김호진"
#define OBJECTMAKER     "최영진 김호진"
...
#define MAX_DROP_ITEM 100 // 무기 최대 드랍 개수 [100개 고정]
#define MAX_INVENTORY 50 // 최대 아이템 소지 개수 10개 한정
#define MAX_NPC 100 // 최대 NPC 수
#define MAX_WEAPON_SLOT 13 // 무기 최대 슬롯
#define MAX_MOVE_OBJECT 30 // 무브오브젝트 최대 개수
#define MAX_ENTER 100 // 최대 입/퇴 개수
#define MAX_GANGZONE 20 + 2 //최대 갱존 개수
#define MAX_OCCUPY_AREA 50 //최대 점령지 개수
#define MAX_PLATOON 30 + 1// 최대 소대 개수
#undef MAX_PLAYERS
#define MAX_PLAYERS 100 //최대 플레이 수
#define MAX_COUNTRY_VEHICLE 100 // 국가 차량 최대 개수
#define MAX_BOMB 5
#define FADE_DELAY 50
#define DIETIME 300 //죽음 시간
#define PickupDown 0.5
#define DROP_ITEM_TIMER 30
#define PILLHEALTH 15.0 // 알약 먹었을 때 오르는 체력
#define MAX_COUNTRY 3
#define Initial_Money 1000 // 국가 초기 금
#define MAX_PRODUCT 50
#define MAX_BONFIRE 100
#define MAX_VEHICLE_MODEL 611 + 1//400 ~ 611 for돌릴 때 400부터 시작.
...
{
	{0,-1,-1,"없음"},{1,331,1,"너클"},{2,333,1,"골프채"},{3,334,1,"경찰봉"},{4,335,1,"나이프"},{5,336,1,"야구배트"},{6,337,1,"삽"},
	{7,338,1,"당구채"},{8,339,1,"카타나"},{9,341,1,"전기톱"},{10,321,1,"보라색 딜도"},{11,322,1,"딜도"},{12,323,1,"전동기"},{13,324,1,"은색 전동기"},
	{14,325,1,"꽃다발"},{15,326,1,"지팡이"},{16,342,-1,"수루탄"},{17,343,-1,"연막탄"},{18,344,-1,"화염병"},{19,-1,-1,"없음"},{20,-1,-1,"없음"},
	{21,-1,-1,"없음"},{22,346,-1,"콜트"},{23,347,-1,"소음 콜트"},{24,348,-1,"데저트 이글"},{25,349,-1,"샷건"},{26, 350,-1,"더블바렐 샷건"},{27,351,-1,"컴벳 샷건"},
	{28,352,-1,"우지"},{29,353,-1,"MP5"},{30,355,-1,"AK-47"},{31,356,-1,"M4-1"},{32,372,-1,"Tec-9"},{33,357,-1,"라이플"},{34,358,-1,"스나이퍼"},{35, 359,-1,"RPG"},
	{36,360,-1,"HS Rocket"},{37,361,-1,"화염방사기"},{38,362,-1,"미니건"},{39,363,-1,"C4"},{40,364,-1,"기폭 장치"},{41,365,-1,"스프레이"},{42,366,-1,"소화기"},
	{43,367,-1,"카메라"},{44,368,1,"고글"},{45,369,1,"고글"},{46,371,1,"낙하산"}, //WeaponID
	{47,1580,5,"모르핀"},{48,2709,5,"알약"},{49,330,1,"무전기"},{50,1252,5,"C4"},{51,2041,300,"예비탄약"},{52,1485,20,"담배"},{53, 1546,5,"맥주"}
};
...
new const MessageText[22][128] =
{
	{"아직 로그인을 하지 않았습니다."},
	{"리더가 아닙니다."},
	{"어드민이 아니거나 어드민 레벨이 부족합니다."},
	{"그 플레이어는 접속하지 않았습니다."},
	{"갱존안에 있지 않거나, 그 갱존안에는 이미 다른 점령지가 있습니다."},
	{"당신이 선택한 플레이어는 아직 접속하지 않았거나 로그인을 하지 않았습니다."},
	{"그 플레이어는 당신과 다른 국가에 소속되어있는 사람입니다."},
	{"사령관이 아닙니다."},
	{"소대장이 아닙니다."},
	{"그 플레이어는 이미 다른 소대의 소대장입니다."},
	{"어드민이 아니거나 소대장이 아닙니다."},
	{"그 플레이어는 다른 소대에 가입되어 있습니다."},
	{"그 플레이어는 이미 다른 소대에 가입되어 있습니다."},
	{"그 플레이어는 당신 가까이 있지 않습니다."},
	{"그 플레이어는 어드민이 아니거나 어드민 레벨이 부족합니다."},
	{"다시 시도해주세요."},
	{"그 플레이어는 기절하지 않았습니다."},
	{"아이템을 소지하지 않고 있거나, 필요한 개수가 부족합니다."},
	{"채팅 금지되었기 때문에 말을 할 수 없습니다."},
	{"소지 하시고계신 달러가 부족합니다."},
	{"가입된 소대가 없습니다."},
	{"기절 상태에서는 할 수 없는 행동입니다."}
};

new const _FactionName[6][_FactionInfo] = //순서, 번호 바꾸는 순간 데이터 꼬임주의 _ 하나도 건들지 말 것.
{
    {"임시소대", 1},
	{"일반소대", 2},
	{"특수소대", 3},
...
public OnGameModeInit()//This callback is triggered when the gamemode starts.
{
	...
	//MOVEOBJECT
	MoveObjectNumber = 0;
	CreateDynamicMoveObject(18755, -1332.23364, 445.75681, 8.12570, 0, 0, 0, -1332.23364, 445.75681, 30.90560, 0, 0, 0,_,_,0,5.0,5.0, "F키를 눌러주세요");//러시아 엘리베이터
	CreateDynamicMoveObject(980, -1527.17493, 482.52319, 8.93010, 0, 0, 0, -1527.17493, 482.52319, 3.39284, 0, 0, 0,_,_,1,3.0,5.0, "F키를 눌러주세요");//러시아 입구
	CreateDynamicMoveObject(980, 91.67800, 1921.15540, 19.77480, 0, 0, 90, 91.67800, 1921.15540, 14.16386, 0, 0, 90,_,_,1,3.0,5.0, "F키를 눌러주세요");//미국 입구

	CreatePickup(1239,1,1213.0793,-246.6340,1021.7993,-1); // 튜토리얼 총 주는 픽업
	CreatePickup(1239,1,296.4462,-38.3054,1001.5156,-1); // 미국 무장 픽업
	CreatePickup(1239,1,295.4839,-80.6940,1001.5156,-1); // 러시아 무장 픽업
	CreatePickup(1239,1,1214.6433,-251.8133,1021.7993,-1);//튜토리얼 픽업
	
	CreateTextPickup(1239, 429.1193,617.2222,1021.4828, ""#C_RED"[!]\n"#C_WHITE"/비상무장\n/기부"); //러시아 임시무장/기부장소
	CreateTextPickup(1239, -226.3625,276.5709,1004.4833, ""#C_RED"[!]\n"#C_WHITE"/비상무장\n/기부"); //미국 임시무장/기부장소
	CreateTextPickup(1239, 202.6366,1887.6580,17.6481, ""#C_WHITE"임시소대 가입\nF키를 눌러주세요."); //미국 임시소대 가입장소
	CreateTextPickup(1239, -1289.2260,440.8935,7.1809, ""#C_WHITE"임시소대 가입\nF키를 눌러주세요."); //러시아 임시소대 가입장소
	...
	//Streamer_MaxItems(STREAMER_TYPE_OBJECT, 900); // 총 버리는 오브젝트가 100개를 차지하므로 900개 로드
	Streamer_VisibleItems(STREAMER_TYPE_OBJECT, 900); // 총 버리는 오브젝트가 100개를 차지하므로 900개 로드
	...
	CreateCountryVehicle(470,234.3007,1902.2339,17.6349,90.0,0,0,1); // 미국험비
	CreateCountryVehicle(470,234.3007,1896.1626,17.6326,90.0,0,0,1); // 미국험비
	CreateCountryVehicle(470,234.3007,1889.8444,17.6339,90.0,0,0,1); // 미국험비
	CreateCountryVehicle(470,234.3007,1884.1542,17.6354,90.0,0,0,1); // 미국험비
	CreateCountryVehicle(470,234.3007,1877.9207,17.6338,90.0,0,0,1); // 미국험비
	CreateCountryVehicle(470,234.3007,1872.0375,17.6338,90.0,0,0,1); // 미국험비
	CreateCountryVehicle(471,282.3509,1907.9001,17.1260,90.0,0,0); // 미국쿼드
	CreateCountryVehicle(471,282.3509,1911.5399,17.1186,90.0,0,0); // 미국쿼드
	CreateCountryVehicle(471,285.3509,1907.9777,17.1292,90.0,0,0); // 미국쿼드
	CreateCountryVehicle(471,285.3509,1911.5936,17.1455,90.0,0,0); // 미국쿼드
	CreateCountryVehicle(424,285.3509,1904.3510,17.4281,90.0,0,0,1); // 미국개구리
	CreateCountryVehicle(424,285.3509,1900.5618,17.4286,90.0,0,0,1); // 미국개구리
	CreateCountryVehicle(424,285.3509,1897.1604,17.4286,90.0,0,0,1); // 미국개구리
	CreateCountryVehicle(525,282.8848,1868.8901,17.5152,90.0,0,0,1); // 미국 토우 트럭
	CreateCountryVehicle(525,282.8848,1865.2587,17.5216,90.0,0,0,1); // 미국 토우 트럭
	CreateCountryVehicle(416,282.8848,1861.6127,17.7874,90.0,1,1,1); // 미국 엠뷸런스
 	CreateCountryVehicle(416,282.8848,1858.0444,17.7966,90.0,1,1,1); // 미국 엠뷸런스
	CreateCountryVehicle(573,282.8848,1854.1163,18.2920,90.0,0,0,1); // 미국 듄
	CreateCountryVehicle(578,280.6710,1849.5992,18.2677,90.0,0,0,1); // 미국 DFT-30
	CreateCountryVehicle(468,285.3509,1895.3684,17.3161,90.0,0,0,1); // 미국산체즈
	CreateCountryVehicle(468,285.3509,1894.0629,17.3161,90.0,0,0,1); // 미국산체즈
	CreateCountryVehicle(596,282.8848,1890.4163,17.3667,90.0,0,0,1); // 미국경찰차
	CreateCountryVehicle(596,282.8848,1886.7815,17.3629,90.0,0,0,1); // 미국경찰차
	CreateCountryVehicle(490,282.8848,1883.1293,17.7853,90.0,0,0,1); // 미국런쳐
	CreateCountryVehicle(490,282.8848,1879.2932,17.7683,90.0,0,0,1); // 미국런쳐
	CreateCountryVehicle(500,285.3509,1876.3203,17.7480,90.0,0,0,1); // 미국장교차
	CreateCountryVehicle(523,285.3509,1874.4119,17.2114,90.0,0,0,1); // 미국오토캅
	CreateCountryVehicle(523,285.3509,1873.2550,17.2114,90.0,0,0,1); // 미국오토캅
	CreateCountryVehicle(523,285.3509,1871.7699,17.2115,90.0,0,0,1); // 미국오토캅
	CreateCountryVehicle(548,350.3255,1978.5098,19.2824,135.0,0,0,1); // 미국카고밥
	CreateCountryVehicle(497,350.3255,1951.4077,17.7816,135.0,0,0,1); // 미국헬기
	CreateCountryVehicle(497,350.3255,1931.2837,17.7826,135.0,0,0,1); // 미국헬기
	CreateCountryVehicle(497,350.3255,1911.6624,17.7783,135.0,0,0,1); // 미국헬기
	CreateCountryVehicle(476,311.8095,2033.0648,18.4062,180.0,0,0,1); // 미국러슬러
	CreateCountryVehicle(476,297.0261,2033.0648,18.4114,180.0,0,0,1); // 미국러슬러
	CreateCountryVehicle(476,311.8095,2048.6477,18.4068,180.0,0,0,1); // 미국러슬러
	CreateCountryVehicle(476,297.0261,2048.6477,18.4080,180.0,0,0,1); // 미국러슬러
	CreateCountryVehicle(511,306.7570,1799.0391,19.0753,0.0,0,0,1); // 미국비글
	
	CreateCountryVehicle(470,-1340.4360,456.4417,7.1781,0.0,0,0,2); // 러시아험비
	CreateCountryVehicle(470,-1343.8666,456.4417,7.1770,0.0,0,0,2); // 러시아험비
	CreateCountryVehicle(470,-1346.9589,456.4417,7.1781,0.0,0,0,2); // 러시아험비
	CreateCountryVehicle(470,-1350.2252,456.4417,7.1803,0.0,0,0,2); // 러시아험비
	CreateCountryVehicle(470,-1353.4426,456.4417,7.1829,0.0,0,0,2); // 러시아험비
	CreateCountryVehicle(470,-1356.6353,456.4417,7.1799,0.0,0,0,2); // 러시아험비
	CreateCountryVehicle(424,-1359.8136,456.4417,6.9679,0.0,0,0,2); // 러시아개구리
	CreateCountryVehicle(424,-1363.5675,456.4417,6.9691,0.0,0,0,2); // 러시아개구리
	CreateCountryVehicle(424,-1367.3159,456.4417,6.9677,0.0,0,0,2); // 러시아개구리
	CreateCountryVehicle(468,-1369.7585,456.4417,6.8567,0.0,0,0,2); // 러시아산체즈
	CreateCountryVehicle(468,-1370.9170,456.4417,6.8566,0.0,0,0,2); // 러시아산체즈
	CreateCountryVehicle(525,-1390.5355,456.4417,7.0599,0.0,0,0,2); // 러시아 토우 트럭
	CreateCountryVehicle(525,-1393.8617,456.4417,7.0637,0.0,0,0,2); // 러시아 토우 트럭
	CreateCountryVehicle(416,-1397.5402,456.4417,7.3753,0.0,0,0,2); // 러시아 엠뷸런스
	CreateCountryVehicle(416,-1401.0262,456.4417,7.3767,0.0,0,0,2); // 러시아 엠뷸런스
	CreateCountryVehicle(573,-1404.9342,456.4417,7.7966,0.0,0,0,2); // 러시아 듄
	CreateCountryVehicle(578,-1408.6306,458.8828,7.8082,0.0,0,0,2); // 러시아 DFT-30
	CreateCountryVehicle(490,-1373.2555,456.4417,7.4093,0.0,0,0,2); // 러시아런쳐
	CreateCountryVehicle(490,-1376.8641,456.4417,7.3151,0.0,0,0,2); // 러시아런쳐
	CreateCountryVehicle(596,-1380.1650,456.4417,6.9078,0.0,0,0,2); // 러시아경찰차
	CreateCountryVehicle(596,-1383.4508,456.4417,6.9077,0.0,0,0,2); // 러시아경찰차
	CreateCountryVehicle(523,-1385.6572,456.4417,6.7582,0.0,0,0,2); // 러시아오토캅
	CreateCountryVehicle(523,-1386.7744,456.4417,6.7576,0.0,0,0,2); // 러시아오토캅
	CreateCountryVehicle(523,-1387.8075,456.4417,6.7437,0.0,0,0,2); // 러시아오토캅
	CreateCountryVehicle(417,-1353.7104,346.0401,30.1806,45.0,0,0,2); // 러시아레비아탄
	CreateCountryVehicle(497,-1369.4349,346.0401,30.2256,45.0,0,0,2); // 러시아헬기
	CreateCountryVehicle(497,-1386.7396,346.0401,30.2251,45.0,0,0,2); // 러시아헬기
	CreateCountryVehicle(497,-1402.0178,346.0401,30.2228,45.0,0,0,2); // 러시아헬기
	CreateCountryVehicle(476,-1457.6907,423.4339,30.8008,270.0,0,0,2); // 러시아러슬러
	CreateCountryVehicle(476,-1457.6907,409.1571,30.8139,270.0,0,0,2); // 러시아러슬러
	CreateCountryVehicle(476,-1473.9941,423.4339,30.8248,270.0,0,0,2); // 러시아러슬러
	CreateCountryVehicle(476,-1473.9941,409.1571,30.8009,270.0,0,0,2); // 러시아러슬러
	CreateCountryVehicle(511,-1448.6418,384.9309,31.5245,270.0,0,0,2); // 러시아비글
	return 1;
}
...
public OnPlayerConnect(playerid)//This callback is called when a player connects to the server.
{
	...
	{
	    if(ServerDBConnect == 0)//MySQL 연결 오류
	    {
	        SendMessage(playerid, "DB오류로 서버에 접속이 불가능 합니다.");
			KickEx(playerid, 1);
			return 1;
		}
		...
		if(rowsnum > 0)
		{
		    for(new num = 0; num < rowsnum; num ++)
		    {
			    ...
			    new string[256];
			    format(string, sizeof(string), ""#C_GRAY"\t- IP밴 - \n\n"#C_WHITE"이름 : %s\n날짜 : %d년 %d월 %d일\n담당 관리자 : %s\n이유 : %s\n\n",
				PlayerNameEx(playerid),year,month,day,AdminName,reason);
				ShowPlayerDialog(playerid, DIALOG_NONE, DIALOG_STYLE_MSGBOX, "The World War", string, "확인", "");
...
public LoginScreen(playerid, screen)
{
    ...
    {
        SendMessage(playerid, "커스텀 오브젝트 검사를 시작합니다.");
        PlayerInfo[playerid][pCustom] = 1;
        ShowPlayerDialog(playerid, DIALOG_CUSTOM, DIALOG_STYLE_MSGBOX, "커스텀 오브젝트 검사", "커스텀 오브젝트가 깔려있는지 검사중입니다.", "확인","");
	...
	else if(screen == 3)
	{
		...
		else
		{
			...
			SendMessage(playerid, "커스텀 패치가 되지 않아 자동으로 킥 당하셨습니다.");
			SendMessage(playerid, "포럼에 가서 커스텀패치를 다운로드 받아주세요.");
			SendMessage(playerid, "포럼 주소 : forum.tww-rp.kro.kr / SAMP weburl 확인");
			TogglePlayerControllable(playerid, 1);
...
public OnPlayerSpawn(playerid)
{
    ...
    if(PlayerInfo[playerid][pTut] == 0)
    {
        ...
        if(PlayerInfo[playerid][pTutDelay] == 0)
        {
            ...
            SendMessage(playerid, "곧 튜토리얼이 시작되니 잠시만 기다려주세요.");
        }
		else if(PlayerInfo[playerid][pTutDelay] == 1)
		{
			ShowPlayerDialog(playerid, DIALOG_SEX, DIALOG_STYLE_LIST, "성별", "남성\n여성", "확인","");
			SendMessage(playerid, "당신의 성별을 정해주세요.");
		}
    }
    ...
    else if(PlayerInfo[playerid][pTut] == 2)
    {
		...
		else if(PlayerInfo[playerid][pDie] > 0)
		{
		    ...
		    else if(PlayerInfo[playerid][pDie] == 2)
		    {
		        ...
		        SendMessage(playerid, "부상당하셨습니다. 3분 이내로 의사가 안 올경우 이 자리에서 출혈로 인해 사망하게 됩니다.");
		        SetCameraBehindPlayer(playerid);
		    }
		    else if(PlayerInfo[playerid][pDie] == 3)
		    {
		        ...
		        TogglePlayerControllable(playerid, 1);
		        SetCameraBehindPlayer(playerid);
		        SendMessage(playerid, "지나가는 로산토스 시민에게 발견되었고, 급히 의무소로 이송되었습니다.");
...
public OnPlayerDeath(playerid, killerid, reason)
{
	if(PlayerInfo[playerid][pTut] == 2)
	{
		...
		else
		{
			switch(reason)
			{
			    case 49 : strmid(weaponname, "차사고", 0, 52, 52);
			    case 50 : strmid(weaponname, "헬리콥터 프로펠러", 0, 52, 52);
			    case 51 : strmid(weaponname, "폭발사고", 0, 52, 52);
			    case 53 : strmid(weaponname, "익사", 0, 52, 52);
			    case 54 : strmid(weaponname, "낙사", 0, 52, 52);
			}
		}
		new Float:Pos[3];
		...
		new string[256];
		format(string, sizeof(string), "%s(%d)님이 %s(%d)님을 사살하였습니다. [Reason : %s]",PlayerNameEx(killerid),killerid, PlayerNameEx(playerid),playerid, weaponname);
...
public OnPlayerCommandText(playerid, cmdtext[])
{
    ...
    {
        format(string, sizeof(string), "당신이 마지막으로 탄 차는 %d번입니다.",PlayerInfo[playerid][pOldVehicle]);
        SendMessage(playerid, string);
        return 1;
    }
    if(strcmp(cmd, "/총버리기", true) == 0)
    {
        DropPlayerItem(playerid,_,_,2);
        return 1;
    }
    if(strcmp(cmd, "/페이데이", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        ...
        format(string, sizeof(string), "[TWWRP]어드민 %s님이 모두에게 페이데이를 실행해주셨습니다.", PlayerInfo[playerid][pAdminName]);
	...
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pPhoneNum] == 0) return SendErrorMessage(playerid, "주파수 설정이 안 됬습니다.");
		...
		if(!strlen(result)) return SendErrorMessage(playerid, "/z [내용]");
	...
	if(strcmp(cmd, "/주파수", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/주파수 [주파수]");
		new num = strval(tmp);
		if(0 < num < 10000)
		{
		    new index = ItemIDFind(playerid, 49);
		    if(index != -1)
		    {
				...
				format(string, sizeof(string), "무전기 주파수가 %d로 맞춰졌습니다.",num);
				SendMessage(playerid, string);
		    }
		    else return SendErrorMessage(playerid, MessageText[17]);
		}
		else return SendErrorMessage(playerid, "주파수는 0 ~ 9999까지 맞출 수 있습니다.");
	    return 1;
	}
	if(strcmp(cmd, "/모닥불", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    ...
	    if(PlayerInfo[playerid][pCampfire] == true)
	    {
			...
			{
			    SendMessage(playerid, "모닥불을 삭제 하였습니다.");
			    DestroyObjectCampfire(playerid);
			}
	        return 1;
	    }
	    else if(PlayerInfo[playerid][pCampfire] == false)
	    {
	    	...
	    	{
	    	    SendMessage(playerid, "모닥불을 만들었습니다.");
	    	    PlayerInfo[playerid][pCampfire] = true;
	    	}
	    	else SendErrorMessage(playerid, "더이상 모닥불을 만들 수 없습니다.");
		}
	    return 1;
	}
	if(strcmp(cmd, "/장교접", true) == 0 || strcmp(cmd, "/장교접속", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
		if(PlayerInfo[playerid][pTut] != 2) return SendErrorMessage(playerid, "튜토리얼 진행중엔 할 수 없는 행동입니다.");
		new str[512];
		...
		ShowPlayerDialog(playerid, DIALOG_NONE, DIALOG_STYLE_LIST, "장교접속", str, "확인","");
        return 1;
    }
	if(strcmp(cmd, "/부사관접", true) == 0 || strcmp(cmd, "/부사관접속", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
		if(PlayerInfo[playerid][pTut] != 2) return SendErrorMessage(playerid, "튜토리얼 진행중엔 할 수 없는 행동입니다.");
		new str[512];
		...
		ShowPlayerDialog(playerid, DIALOG_NONE, DIALOG_STYLE_LIST, "부사관접속", str, "확인","");
        return 1;
    }
    if(strcmp(cmd, "/소대원접", true) == 0 || strcmp(cmd, "/소대원접속", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
		if(PlayerInfo[playerid][pTut] != 2) return SendErrorMessage(playerid, "튜토리얼 진행중엔 할 수 없는 행동입니다.");
		new str[512];
		...
		ShowPlayerDialog(playerid, DIALOG_NONE, DIALOG_STYLE_LIST, "소대원접속", str, "확인","");
        return 1;
    }
    if(strcmp(cmd, "/소대탈퇴", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        if(PlayerInfo[playerid][pPlatoonType] == 0) return SendErrorMessage(playerid, MessageText[20]);
        if(PlayerInfo[playerid][pPlatoonType] != 1) return SendErrorMessage(playerid, "임시소대 외엔 할 수 없는 행동입니다.");
		if(PlayerInfo[playerid][pWork] == 1) return SendErrorMessage(playerid, "무장을 해제한 뒤 시도해주세요.");
        SetPlayerPlatoonLevel(playerid, 0, 0);
		SendMessage(playerid, "당신은 임시소대를 탈퇴 하였습니다.");
		return 1;
    }
    if(strcmp(cmd, "/비상무장", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        tmp = strtok(cmdtext, idx);
        if(!strlen(tmp)) return SendErrorMessage(playerid, "/비상무장");
        if(PlayerInfo[playerid][pCountry] == 1)
        {
            if(IsPlayerInRangeOfPoint(playerid, 1.0, -226.3625,276.5709,1004.4833))
            {
            }
            else return SendErrorMessage(playerid, "비상무장 할 수 있는 장소에 있지 않습니다.");
        }
        else if(PlayerInfo[playerid][pCountry] == 2)
        {
    ...
    if(strcmp(cmd, "/기부", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        tmp = strtok(cmdtext, idx);
        if(!strlen(tmp)) return SendErrorMessage(playerid, "/기부 [금액]");
        ...
        if(0 < money <= PlayerInfo[playerid][pMoney])
        {
	        if(PlayerInfo[playerid][pCountry] == 1)
	        {
	            if(IsPlayerInRangeOfPoint(playerid, 1.0, -226.3625,276.5709,1004.4833))
	            {
	                ...
	                format(string, sizeof(string), "국가에 %d$를 기부하였습니다.",money);
	                SendMessage(playerid, string);
	                format(string, sizeof(string), "[%s]%s님이 국가에 %d$를 기부하였습니다.",ArmyClass(PlayerInfo[playerid][pClass]),PlayerNameEx(playerid), money);
	                SendCountryMessage(string, 1, HexToInt(COLOR_USA));
	                Save_Server_Info();
	            }
	            else return SendErrorMessage(playerid, "기부 할 수 있는 장소에 있지 않습니다.");
	        }
	        else if(PlayerInfo[playerid][pCountry] == 2)
	        {
	        	if(IsPlayerInRangeOfPoint(playerid, 1.0, 429.1193,617.2222,1021.4828))
    ...
    if(strcmp(cmd, "/기부금액", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
		format(string, sizeof(string), "기부한 금액 : %d$",PlayerInfo[playerid][pDonation]);
		SendMessage(playerid, string);
        return 1;
    }
    if(strcmp(cmd, "/bd", true) == 0 || strcmp(cmd, "/소대무전", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/소대무전 [내용]");
		PlatoonRadio(playerid, result);
        return 1;
    }
	if(strcmp(cmd, "/담배", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    new index = ItemIDFind(playerid, 52);
		if(index == -1) return SendErrorMessage(playerid, MessageText[17]);
		if(PlayerInfo[playerid][pSmokeStart] == 1) return SendErrorMessage(playerid, "이미 담배를 피우고있습니다.");
		...
		OnPlayerCommandText(playerid, "/me 주머니에서 담배 한개를 꺼내 불을 붙인다.");
	    return 1;
	}
	if(strcmp(cmd, "/맥주", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    new index = ItemIDFind(playerid, 53);
		if(index == -1) return SendErrorMessage(playerid, MessageText[17]);
		if(PlayerInfo[playerid][pBeerStart] == 1) return SendErrorMessage(playerid, "이미 맥주를 들고있습니다.");
	    RemoveItemEx(playerid, 53, 1);
		PlayerInfo[playerid][pBeerStart] = 1;
		SetPlayerSpecialAction(playerid, SPECIAL_ACTION_DRINK_WINE);
		OnPlayerCommandText(playerid, "/me 가방에 있던 맥주 한캔을 꺼낸다.");
	    return 1;
	}
	if(strcmp(cmd, "/자가방탄복", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pCorps] != 1) return SendErrorMessage(playerid, "전투병이 아닙니다.");
	    if(PlayerInfo[playerid][pArmour] >= 100) return SendErrorMessage(playerid, "당신은 이미 아머가 꽉 찻습니다.");
		if(PlayerInfo[playerid][pAmourTimer] <= 0)
		{
            PlayerInfo[playerid][pAmourTimer] = 10;
			GivePlayerArmourEx(playerid, 30);
			format(string, sizeof(string), "방탄복을 재점검 하였습니다.");
		    SendMessage(playerid, string);
		}
		else if(PlayerInfo[playerid][pAmourTimer] > 0)
		{
		    format(string, sizeof(string), "다시 시도해주세요 [%d초]", PlayerInfo[playerid][pAmourTimer]);
			SendErrorMessage(playerid, string);
		}
	    return 1;
	}
	if(strcmp(cmd, "/방탄복", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pCorps] != 1) return SendErrorMessage(playerid, "전투병이 아닙니다.");
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/방탄복 [플레이어번호]");
	    ...
	    if(PlayerInfo[giveplayerid][pArmour] >= 100) return SendErrorMessage(playerid, "그 플레이어는 이미 아머가 100.0입니다.");
		if(PlayerInfo[playerid][pAmourTimer] <= 0)
		{
            PlayerInfo[playerid][pAmourTimer] = 10;
			GivePlayerArmourEx(giveplayerid, 30);
			format(string, sizeof(string), "%s님의 아머를 30.0올렷습니다.",PlayerNameEx(giveplayerid));
		    SendMessage(playerid, string);
			format(string, sizeof(string), "%s님에 의해 아머가 30.0올랏습니다.",PlayerNameEx(playerid));
	...
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pCorps] != 2) return SendErrorMessage(playerid, "공병이 아닙니다.");
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp))
		{
			SendErrorMessage(playerid, "/바리게이트제거 [1~2]");
			SendErrorMessage(playerid, "1 : 바리게이트, 2 : 스파이크");
			return 1;
		}
		new num = strval(tmp);
	    if(0 < num < 3)
	    {
	        if(PlayerInfo[playerid][pBarricade][num] == 1)
	        {
	            ...
	            if(IsPlayerInRangeOfPoint(playerid, 2.0, pos[0], pos[1], pos[2]))
	            {
		            ...
		            SendMessage(playerid, "바리게이트를 제거하였습니다.");
				}
				else return SendErrorMessage(playerid, "바리게이트 가까이에서 다시 시도해주세요.");
	        }
	        else return SendErrorMessage(playerid, "바리게이트를 만들지 않았습니다.");
	    }
		else return SendErrorMessage(playerid, "바리게이트는 1 ~ 2까지 있습니다.");
		return 1;
	}
	if(strcmp(cmd, "/바리게이트", true) == 0 || strcmp(cmd, "/바리", true) == 0)
	{
	    ...
	    if(!strlen(tmp))
		{
			SendErrorMessage(playerid, "/바리게이트 [1~2]");
			SendErrorMessage(playerid, "1 : 바리게이트, 2 : 스파이크");
			return 1;
		}
	    new num = strval(tmp);
	    if(0 < num < 3)
	    {
			if(PlayerInfo[playerid][pBarricade][num] == 0)
		    {
			    ...
			    format(string, sizeof(string), "%s가 만든\n%s",PlayerNameEx(playerid),name);
				PlayerInfo[playerid][pBarricadeLabel][num] = Create3DTextLabel(string,COLOR_GRAY,pos[0], pos[1], pos[2]+0.3,5.0,0);
			    SendMessage(playerid, "바리게이트를 만들었습니다.");
			}
			else if(PlayerInfo[playerid][pBarricade][num] == 1) return SendErrorMessage(playerid, "바리게이트 및 스파이크는 각 각 한개씩만 만들 수 있습니다.");
		}
		else return SendErrorMessage(playerid, "바리게이트는 1 ~ 2까지 있습니다.");
	    return 1;
	}
	if(strcmp(cmd, "/쌀포대", true) == 0 || strcmp(cmd, "/흙포대", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pCorps] != 1) return SendErrorMessage(playerid, "전투병이 아닙니다.");
		...
		else if(PlayerInfo[playerid][pSoliderBag] == 1)
		{
		    ...
		    if(IsPlayerInRangeOfPoint(playerid, 2.0, pos[0], pos[1], pos[2]))
		    {
		    	...
		    	PlayerInfo[playerid][pSoliderBagObject] = -1;
		    	Delete3DTextLabel(PlayerInfo[playerid][pSoliderBagLabel]);
		    	SendMessage(playerid, "쌀포대를 제거하였습니다.");
			}
			else return SendErrorMessage(playerid, "쌀포대 가까이에서 다시 시도해주세요.");
		}
	    return 1;
	}
	if(strcmp(cmd, "/모르핀", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    new Inventory = -1;
	    for(new i = 0; i < MAX_INVENTORY; i ++)
	    {
	    	...
	    	else if(i == MAX_INVENTORY - 1) return SendErrorMessage(playerid, "모르핀을 소유하고 있지 않습니다.");
	    }
	    if(Inventory == -1) return SendErrorMessage(playerid, "모르핀을 소유하고 있지 않습니다.");
		tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/모르핀 [플레이어번호]");
	    new giveplayerid = strval(tmp);
     	if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    if(!IsGivePlayerDistance(3.0, playerid, giveplayerid)) return SendErrorMessage(playerid, MessageText[13]);
		if(PlayerInfo[giveplayerid][pDie] != 2) return SendErrorMessage(playerid, MessageText[16]);
		format(string, sizeof(string), "/me %s(이)에게 모르핀을 투여 한다.",PlayerNameEx(giveplayerid));
		OnPlayerCommandText(playerid, string);
		format(string, sizeof(string), "/me 모르핀을 맞고 간신히 일어난다.");
	...
	if(strcmp(cmd, "/폭탄설치", true) == 0)
	{
	    ...
	    if(PlayerInfo[playerid][pC4Emplace] == 0)
		{
		    for(new o = 0; o < MAX_BOMB; o ++)
			{
				...
				else if(o == MAX_BOMB-1) return SendErrorMessage(playerid, "5개 이상 설치할 수 없습니다.");
			}
		    C4EmplaceMode(playerid, true);
		    SendMessage(playerid, "C4설치모드가 활성화 되었습니다. 사용방법 : [/도움말]");
		}
		else return SendErrorMessage(playerid, "이미 C4설치모드가 활성화 되었습니다.");
		return 1;
	}
	if(strcmp(cmd, "/폭파", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    new bomb = 0;
	    for(new o = 0; o < MAX_BOMB; o ++)
		{
			...
			else if(o == MAX_BOMB-1 && bomb == 0) return SendErrorMessage(playerid, "설치된 C4가 없습니다.");
		}
		return 1;
	}
	if(strcmp(cmd, "/ip", true) == 0)
	{
	    ...
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/ip [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    new PlayerIP[32];
	    GetPlayerIp(giveplayerid, PlayerIP, sizeof(PlayerIP));
	    format(string, sizeof(string), "%s님의 아이피는 %s입니다.",PlayerNameEx(giveplayerid), PlayerIP);
	...
	if(strcmp(cmd, "/인테리어", true) == 0)
	{
	    ...
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/인테리어 [플레이어번호] [인테리어]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/인테리어 플레이어번호 [인테리어]");
	    new interior = strval(tmp);
	    format(string, sizeof(string), "어드민 %s님이 %s님의 인테리어 번호를 %d로 변경하였습니다.",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), interior);
		SendAdminMessage(string);
		format(string, sizeof(string), "어드민 %s님에 의해 인테리어 번호가 %d로 변경되었습니다.", PlayerInfo[playerid][pAdminName], PlayerInfo[playerid][pAdminName], interior);
		SendMessage(giveplayerid, string);
		SetPlayerInteriorEx(giveplayerid, interior);
	    return 1;
	}
	if(strcmp(cmd, "/버추얼월드", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/버추얼월드 [플레이어번호] [월드]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/버추얼월드 플레이어번호 [월드]");
	    new virtualworld = strval(tmp);
	    format(string, sizeof(string), "어드민 %s님이 %s님의 버추얼월드를 %d로 변경하였습니다.",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), virtualworld);
		SendAdminMessage(string);
		format(string, sizeof(string), "어드민 %s님에 의해 버추얼 월드가 %d로 변경되었습니다.", PlayerInfo[playerid][pAdminName], PlayerInfo[playerid][pAdminName], virtualworld);
		SendMessage(giveplayerid, string);
		SetPlayerVirtualWorldEx(giveplayerid, virtualworld);
	    return 1;
	}
	if(strcmp(cmd, "/점령지현황", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    //if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
		new str[512], country[12], Occupy[12];
		strmid(str, ""#C_GRAY"번호\t국가\t점령가동"#C_WHITE"", 0, 512, 512);
	    for(new o = 0; o < MAX_OCCUPY_AREA; o ++)
	    {
	        if(Occupy_AreaInfo[o][oCreate] == 1)
	        {
	            if(Occupy_AreaInfo[o][oCountry] == 0) country = "무점령";
	            else if(Occupy_AreaInfo[o][oCountry] == 1) country = "미국";
	            else if(Occupy_AreaInfo[o][oCountry] == 2) country = "러시아";

	            if(Occupy_AreaInfo[o][oOccupy] == 0) Occupy = "불가";
	            else if(Occupy_AreaInfo[o][oOccupy] == 1) Occupy = "가능";

	            format(str, sizeof(str), "%s\n%d\t%s\t%s",str, Occupy_AreaInfo[o][oNumber], country, Occupy);
			}
	    }
	    ShowPlayerDialog(playerid, DIALOG_NONE, DIALOG_STYLE_LIST, "점령지현황", str, "확인", "");
	    return 1;
	}
	if(strcmp(cmd, "/점령가동", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/점령가동 [점령지번호]");
	    new OccupyID = strval(tmp);
	    if(Occupy_AreaInfo[OccupyID][oCreate] == 0) return SendErrorMessage(playerid, "없는 점령지입니다. [/점령지현황]");
	    if(Occupy_AreaInfo[OccupyID][oOccupy] == 1) return SendErrorMessage(playerid, "이미 점령지가 가동된 지역입니다.");
	    Occupy_AreaInfo[OccupyID][oOccupy] = 1;
	    format(string, sizeof(string), "어드민 %s님에 %s점령지가 가동되었습니다.",PlayerInfo[playerid][pAdminName], Occupy_AreaInfo[OccupyID][oName]);
		SendOOCNoticeMessage(string);
		SaveOccupyArea(OccupyID);
		return 1;
	}
	if(strcmp(cmd, "/점령차단", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/점령차단 [점령지번호]");
	    new OccupyID = strval(tmp);
	    if(Occupy_AreaInfo[OccupyID][oCreate] == 0) return SendErrorMessage(playerid, "없는 점령지입니다. [/점령지현황]");
	    if(Occupy_AreaInfo[OccupyID][oOccupy] == 0) return SendErrorMessage(playerid, "이미 점령지가 차단된 지역입니다.");
	    Occupy_AreaInfo[OccupyID][oOccupy] = 0;
	    format(string, sizeof(string), "어드민 %s님에 %s점령지가 차단되었습니다.",PlayerInfo[playerid][pAdminName], Occupy_AreaInfo[OccupyID][oName]);
		SendOOCNoticeMessage(string);
		SaveOccupyArea(OccupyID);
		return 1;
	}
	if(strcmp(cmd, "/차리스폰", true) == 0)
	{
	    ...
	    if(IsPlayerInAnyVehicle(playerid))
	    {
	        ...
	        format(string, sizeof(string), "당신은 %d번 차량을 리스폰하였습니다.", VehicleID);
	        SendMessage(playerid, string);
	    }
	    else
	    {
	        tmp = strtok(cmdtext, idx);
	        if(!strlen(tmp)) return SendErrorMessage(playerid, "/차리스폰 [차번호]");
	...
	if(strcmp(cmd, "/모든차리스폰", true) == 0 || strcmp(cmd, "/모차리", true) == 0)
	{
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
		...
		format(string, sizeof(string), "어드민 %s님이 모든차량을 리스폰시켯습니다.",PlayerInfo[playerid][pAdminName]);
		SendOOCNoticeMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/모든차기름", true) == 0 || strcmp(cmd, "/모차기", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    for(new v = 0; v < MAX_VEHICLES; v ++)
	...
	if(strcmp(cmd, "/위성", true) == 0)
	{
	    ...
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/위성 [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    if(PlayerInfo[playerid][pSatellite] == 1) return SendErrorMessage(playerid, "위성을 끄고 다시 시도해주세요. [/위성종료]");
	...
	if(strcmp(cmd, "/텔레포트", true) == 0)
	{
	    ...
	    ShowPlayerDialog(playerid, DIALOG_TELEPORT, DIALOG_STYLE_LIST, "텔레포트", "국가이동\n점령지이동", "확인", "취소");
	    return 1;
	}
	if(strcmp(cmd, "/위성종료", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    if(!PlayerInfo[playerid][pSatellite]) return SendErrorMessage(playerid, "위성은 이미 종료되있는 상태입니다.");
	    SetPlayerInteriorEx(playerid, PlayerInfo[playerid][pInterior]);
	    SetPlayerVirtualWorldEx(playerid, PlayerInfo[playerid][pVirtualWorld]);
	...
	if(strcmp(cmd, "/무기", true) == 0)
	{
	    ...
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/무기 [플레이어번호] [무기번호] [탄창]");
		new giveplayerid = strval(tmp);
		if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/무기 플레이어번호 [무기번호] [탄창]");
		...
		format(string, sizeof(string), "어드민 %s님이 %s님에게 %s를 %d발 주었습니다.",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), weaponname, ammo);
		SendAdminMessage(string);
		format(string, sizeof(string), "어드민 %s님에 의해 %s가 %d발 추가되었습니다.", PlayerInfo[playerid][pAdminName], weaponname, ammo);
		SendMessage(giveplayerid, string);
		GivePlayerWeaponEx(giveplayerid, weaponid, ammo);
		return 1;
	}
	if(strcmp(cmd, "/시간", true) == 0)
	{
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        if(PlayerInfo[playerid][pDeMorgan]) format(string, sizeof(string), "드모르간 : %d초 ", PlayerInfo[playerid][pDeMorganTimer]);
		if(PlayerInfo[playerid][pJail] == 1) format(string, sizeof(string), "%s영창 : %d초 ",string, PlayerInfo[playerid][pJailTimer]);
		if(PlayerInfo[playerid][pJail] == 2) format(string, sizeof(string), "%s감옥 : %d초",string, PlayerInfo[playerid][pJailTimer]);
        if(strlen(string)) SendMessage(playerid, string);
	    return 1;
	}
	if(strcmp(cmd, "/경고차감", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/경고차감 [플레이어번호] [이유]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/경고차감 플레이어번호 [이유]");
		format(string, sizeof(string), "어드민 %s님이 %s님의 경고를 차감하였습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), result);
		SendAdminMessage(string, COLOR_WARNING);
		format(string, sizeof(string), "어드민 %s님에게 경고를 차감받았습니다. [이유 : %s]", PlayerInfo[playerid][pAdminName], result);
	...
	if(strcmp(cmd, "/스킨변경", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/스킨변경 [플레이어번호] [스킨번호]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/스킨변경 플레이어번호 [스킨번호]");
		new skin = strval(tmp);
		format(string, sizeof(string), "%s님의 스킨을 %d로 바꾸었습니다.", PlayerNameEx(giveplayerid), skin);
		SendMessage(playerid, string);
		format(string, sizeof(string), "어드민 %s님에 의해 스킨이 %d로 바뀌었습니다.", PlayerInfo[playerid][pAdminName], skin);
		SendMessage(giveplayerid, string);
		SetPlayerSkinEx(giveplayerid, skin);
	    return 1;
	}
	if(strcmp(cmd, "/ip밴", true) == 0 || strcmp(cmd, "/아이피밴", true) == 0 || strcmp(cmd, "/ipban", true) == 0 || strcmp(cmd, "/밴추가", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/ip밴 [ip]");
		format(string, sizeof(string), "어드민 %s님이 IP밴을 하였습니다. [IP : %s]",PlayerInfo[playerid][pAdminName], result);
		SendAdminMessage(string, COLOR_WARNING);
		BanAdd(_,result, "밴 추가", playerid, 3);
	    return 1;
	}
	if(strcmp(cmd, "/광역밴", true) == 0)
	{
		if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 4) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/광역밴 [플레이어번호] [이유]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/광역밴 플레이어번호 [이유]");
		format(string, sizeof(string), "어드민 %s님이 %s님을 광역밴 처리 하였습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), result);
		SendAdminMessage(string, COLOR_WARNING);
		format(string, sizeof(string), "어드민 %s님에 의해 광역밴 처리 당하셨습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], result);
		SendMessage(giveplayerid, string, COLOR_WARNING);
		BanAdd(giveplayerid, _, result, playerid, 2);
		return 1;
	}
	if(strcmp(cmd, "/밴", true) == 0 || strcmp(cmd, "/영구밴", true) == 0 || strcmp(cmd, "/영밴", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/영구밴 [플레이어번호] [이유]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/영구밴 플레이어번호 [이유]");
		format(string, sizeof(string), "어드민 %s님이 %s님을 영구밴 처리하였습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), result);
		SendAdminMessage(string, COLOR_WARNING);
		format(string, sizeof(string), "어드민 %s님에 의해 영구밴 처리 당하셨습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], result);
		SendMessage(giveplayerid, string, COLOR_WARNING);
		BanAdd(giveplayerid, _, result, playerid, 1);
		return 1;
	}
	if(strcmp(cmd, "/밴해제", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx); // strtok result value 24
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/밴해제 [플레이어이름/IP]");
		...
		{
		    format(string, sizeof(string), "어드민 %s님이 %s의 영구밴을 해제하였습니다.",PlayerInfo[playerid][pAdminName], tmp);
			SendAdminMessage(string, COLOR_WARNING);
		...
		else SendErrorMessage(playerid, "존재하지 않는 플레이어 정보입니다.");
		cache_delete(result);
		return 1;
	}
	if(strcmp(cmd, "/아이디밴", true) == 0 || strcmp(cmd, "/아밴", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/아이디밴 [플레이어번호] [이유]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/아이디밴 플레이어번호 [이유]");
		format(string, sizeof(string), "어드민 %s님이 %s님의 계정을 정지시켯습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), result);
		SendAdminMessage(string, COLOR_WARNING);
		IDBan(giveplayerid, playerid, result);
		return 1;
	}
	if(strcmp(cmd, "/아이디밴해제", true) == 0 || strcmp(cmd, "/아밴해제", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/아이디밴해제 [플레이어번호/이름]");
		new giveplayerid = strval(tmp);
		if(giveplayerid == INVALID_PLAYER_ID && IsPlayerConnected(giveplayerid))
		{
		    if(PlayerInfo[giveplayerid][pIDBan] == 0) return SendErrorMessage(playerid, "그 플레이어는 아이디밴 상태가 아닙니다.");

		    format(string, sizeof(string), "어드민 %s님이 %s님의 계정 정지를 해제하였습니다.",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
			SendAdminMessage(string, COLOR_WARNING);
		...
		if(cache_num_rows(MySQL) > 0)
		{
		    format(string, sizeof(string), "어드민 %s님이 %s님의 계정 정지를 해제하였습니다. 3",PlayerInfo[playerid][pAdminName], tmp);
			SendAdminMessage(string, COLOR_WARNING);
		...
		else SendErrorMessage(playerid, "존재하지 않는 플레이어 정보이거나 아이디밴을 당하지 않은 플레이어입니다.");
		cache_delete(result);
		return 1;
	}
	if(strcmp(cmd, "/칭찬차감", true) == 0)
	{
	    ...
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/칭찬차감 [플레이어번호] [이유]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/칭찬차감 플레이어번호 [이유]");
		format(string, sizeof(string), "어드민 %s님이 %s님의 칭찬을 차감하였습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), result);
		SendAdminMessage(string, COLOR_PRAISE);
		format(string, sizeof(string), "어드민 %s님에게 칭찬을 차감받았습니다. [이유 : %s]", PlayerInfo[playerid][pAdminName], result);
	...
	if(strcmp(cmd, "/칭찬", true) == 0)
	{
	    ...
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
     	tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/칭찬 [플레이어번호] [이유]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/칭찬 플레이어번호 [이유]");
		format(string, sizeof(string), "어드민 %s님이 %s님에게 칭찬을 해주었습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), result);
		SendAdminMessage(string, COLOR_PRAISE);
		format(string, sizeof(string), "어드민 %s님에게 칭찬을 받았습니다. [이유 : %s]", PlayerInfo[playerid][pAdminName], result);
		SendMessage(giveplayerid, string, COLOR_PRAISE);
		PlayerInfo[giveplayerid][pPraise] ++;
	    return 1;
	}
	if(strcmp(cmd, "/경고", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
     	tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/경고 [플레이어번호] [이유]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/경고 플레이어번호 [이유]");
		format(string, sizeof(string), "어드민 %s님이 %s님에게 경고를 주었습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), result);
		SendAdminMessage(string, COLOR_WARNING);
		format(string, sizeof(string), "어드민 %s님에게 경고를 받았습니다. [이유 : %s]", PlayerInfo[playerid][pAdminName], result);
		SendMessage(giveplayerid, string, COLOR_WARNING);
		PlayerInfo[giveplayerid][pWarning] ++;
		if(PlayerInfo[giveplayerid][pWarning] >= 10)
		{
		    PlayerInfo[giveplayerid][pWarning] = 10;
		    IDBan(giveplayerid, playerid, "경고 10개 누적");
		    format(string, sizeof(string), "%님이 경고가 누적되어 10개를 넘었으므로 자동으로 아이디밴 처리 당하셨습니다.", PlayerNameEx(giveplayerid));
			SendAdminMessage(string, COLOR_WARNING);
		    format(string, sizeof(string), "경고가 누적되어 10개가 넘었으므로 자동으로 아이디밴 되었습니다.");
			SendMessage(giveplayerid, string, COLOR_WARNING);
		}
	    return 1;
	}
	if(strcmp(cmd, "/드모르간확인", true) == 0 || strcmp(cmd, "/드모확인", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
		...
		ShowPlayerDialog(playerid, DIALOG_DEMORGAN, DIALOG_STYLE_LIST, "드모르간 확인", str, "확인", "");
	    return 1;
	}
	if(strcmp(cmd, "/드모르간", true) == 0 || strcmp(cmd, "/드모", true) == 0)
	{
	    ...
	    if(PlayerInfo[playerid][pAdmin] < 1) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/드모르간 [플레이어번호] [시간/분]");
		new giveplayerid = strval(tmp);
		if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		if(PlayerInfo[giveplayerid][pJail] > 0) return SendErrorMessage(playerid, "그 플레이어는 이미 감옥에 있기 때문에 드모르간에 보낼 수 없습니다.");
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/드모르간 플레이어번호 [시간/분]");
		new minute = strval(tmp);
		format(string, sizeof(string), "어드민 %s님이 %s님에게 드모르간에 보냈습니다. [%d분]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), minute);
		SendAdminMessage(string);
		format(string, sizeof(string), "어드민 %s님에 의해 드모르간에 %d분동안 갇힙니다.", PlayerInfo[playerid][pAdminName], minute);
		SendMessage(giveplayerid, string);
		PlayerInfo[giveplayerid][pDeMorgan] = 1;
		PlayerInfo[giveplayerid][pDeMorganTimer] = minute * 60;
	    return 1;
	}
	if(strcmp(cmd, "/관리자레벨", true) == 0 || strcmp(cmd, "/어드민레벨", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 5) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/관리자레벨 [플레이어번호] [레벨]");
	    new giveplayerid = strval(tmp);
		if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/관리자레벨 플레이어번호 [레벨]");
	    new level = strval(tmp);
	    format(string, sizeof(string), "%s(%d)님에게 관리자 %d레벨을 주었습니다.",PlayerNameEx(giveplayerid), giveplayerid, level);
		SendMessage(playerid, string);
		format(string, sizeof(string), "어드민 %s님에 의해 관리자레벨이 %d로 수정되었습니다.", PlayerInfo[playerid][pAdminName], level);
		SendMessage(giveplayerid, string);
		PlayerInfo[giveplayerid][pAdmin] = level;
		printf("어드민 %s님이 %s(%d)님에게 관리자 %d레벨을 주었습니다.",PlayerInfo[playerid][pAdminName],PlayerNameEx(giveplayerid), giveplayerid,level);
		return 1;
	}
	if(strcmp(cmd, "/귓말검사", true) == 0 || strcmp(cmd, "/귓속말검사", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    if(PlayerInfo[playerid][pCh_Whisper] == 0)
	    {
	        SendMessage(playerid, "귓속말 검사를 시작합니다.");
	        PlayerInfo[playerid][pCh_Whisper] = 1;
	    }
	    else if(PlayerInfo[playerid][pCh_Whisper] == 1)
	    {
	        SendMessage(playerid, "귓속말 검사를 종료하였습니다.");
	        PlayerInfo[playerid][pCh_Whisper] = 0;
	    }
	    return 1;
	}
	if(strcmp(cmd, "/총검사", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/총검사 [플레이어번호]");
		new giveplayerid = strval(tmp);
		if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		...
		for(new w = 0; w < MAX_WEAPON_SLOT; w ++)
		{
			...
			{
			    index++;
			    if(index == 1) { format(string, sizeof(string), "%s %d발", GetWeaponNameEx(weaponid[w]), ammo[w]); continue; }
			    if(strlen(string)) { format(string, sizeof(string), "%s | %s %d발",string, GetWeaponNameEx(weaponid[w]), ammo[w]); continue; }
	...
	if(strcmp(cmd, "/정보확인", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/어드민이름 [플레이어번호] [이름]");
		new giveplayerid = strval(tmp);
		if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	...
	if(strcmp(cmd, "/질문채널", true) == 0)
	{
	    ...
	    if(NewbieChannel == 1)
	    {
	        format(string, sizeof(string), "어드민 %s님에 의해 질문채널이 닫혓습니다.",PlayerInfo[playerid][pAdminName]);
	        SendOOCNoticeMessage(string);
	        NewbieChannel = 0;
	    }
	    if(NewbieChannel == 0)
	    {
	        format(string, sizeof(string), "어드민 %s님에 의해 질문채널이 열렷습니다.",PlayerInfo[playerid][pAdminName]);
	        SendOOCNoticeMessage(string);
	        NewbieChannel = 1;
	    }
		return 1;
	}
	if(strcmp(cmd, "/어드민이름", true) == 0 || strcmp(cmd, "/관리닉", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 4) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/어드민이름 플레이어번호 [이름]");
		format(string, sizeof(string), "어드민 %s님의 어드민 이름을 %s로 설정 하였습니다.",PlayerInfo[giveplayerid][pAdminName], result);
		SendMessage(playerid, string);
		format(string, sizeof(string), "어드민 %s님에 의해 어드민 이름이 %s로 설정 되었습니다.", PlayerInfo[playerid][pAdminName], result);
		SendMessage(giveplayerid, string);
		printf("어드민 %s님이 %s님의 어드민 이름을 %s로 설정 하였습니다.",PlayerInfo[playerid][pAdminName],PlayerInfo[giveplayerid][pAdminName], result);
		strmid(PlayerInfo[giveplayerid][pAdminName], result, 0, 128, 128);
		return 1;
	}
	if(strcmp(cmd, "/공지", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
        new length = strlen(cmdtext);
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/공지 [내용]");
	...
	if(strcmp(cmd, "/me", true) == 0 || strcmp(cmd, "/do", true) == 0 || strcmp(cmd, "/행동", true) == 0)
	{
		...
		format(string, sizeof(string), "* (행동) %s %s", PlayerNameEx(playerid, 2), result);
	...
	if(strcmp(cmd, "/c", true) == 0 || strcmp(cmd, "/조용히", true) == 0)
	{
		...
		format(string, sizeof(string), "%s작게 말함 %s..", PlayerNameEx(playerid, 2), result);
	...
	if(strcmp(cmd, "/s", true) == 0 || strcmp(cmd, "/외치다", true) == 0)
	{
		...
		format(string, sizeof(string), "%s 외침 %s!", PlayerNameEx(playerid, 2), result);
		SendAroundMessage(playerid, 15.0, COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE4, COLOR_FADE5, string);
		return 1;
	}
	if(strcmp(cmd, "/st", true) == 0 || strcmp(cmd, "/상태", true) == 0)
	{
		...
		format(string, sizeof(string), "* (상태) %s %s", PlayerNameEx(playerid, 2), result);
	...
	if(strcmp(cmd, "/채팅금지", true) == 0 || strcmp(cmd, "/채금", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/채팅금지 [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    if(!PlayerInfo[giveplayerid][pChatBan])
	    {
		    format(string, sizeof(string), "어드민 %s님이 %s님에게 채팅금지를 당하셨습니다.",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
			SendAdminMessage(string);
			format(string, sizeof(string), "어드민 %s님에 의해 채팅금지가 되었습니다.", PlayerInfo[playerid][pAdminName]);
			SendMessage(giveplayerid, string);
			PlayerInfo[giveplayerid][pChatBan] = 1;
		}
		else if(PlayerInfo[giveplayerid][pChatBan] == 1)
		{
		    format(string, sizeof(string), "어드민 %s님이 %s님의 채팅금지를 풀었습니다.",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
			SendAdminMessage(string);
			format(string, sizeof(string), "어드민 %s님에 의해 채팅금지가 풀렷습니다.", PlayerInfo[playerid][pAdminName]);
			SendMessage(giveplayerid, string);
	...
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return SendErrorMessage(playerid, "차량 운전석에 탑승해주세요.");
	    new vehicleid = GetPlayerVehicleID(playerid);
		if(IsPlayerVehicleStart(playerid, vehicleid) == 0) return SendErrorMessage(playerid, "당신이 시동 걸 수 없는 차량입니다.");
	    return 1;
	}
	if(strcmp(cmd, "/응급", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pCorps] != 3) return SendErrorMessage(playerid, "의무병이 아닙니다.");
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) SendErrorMessage(playerid, "/응급 [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    if(!IsGivePlayerDistance(3.0, playerid, giveplayerid)) return SendErrorMessage(playerid, MessageText[13]);
	    if(PlayerInfo[giveplayerid][pDie] != 2) return SendErrorMessage(playerid, MessageText[16]);
	    if(PlayerInfo[playerid][pSmokeStart] == 1) return SendErrorMessage(playerid, "담배를 꺼주세요.");
	    if(PlayerInfo[playerid][pBeerStart] == 1) return SendErrorMessage(playerid, "맥주를 버려주세요.");
	    if(PlayerInfo[playerid][pFirstaid] == 1) return SendErrorMessage(playerid, "당신은 이미 응급처치를 시도하고 있습니다.");
		SendMessage(playerid, "스페이스바를 연타해주세요.");
		format(string, sizeof(string), "/me 어깨는 %s의 흉골이 맞닿는 부위와 수직이 되게 위치하게 하고, 양쪽 어깨 힘을 이용하여 100회 눌러준다.",PlayerNameEx(giveplayerid));
		OnPlayerCommandText(playerid, string);
	...
	if(strcmp(cmd, "/관응", true) == 0 ||strcmp(cmd, "/관리자응급", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/관리자응급 [플레이어번호]");
		new giveplayerid = strval(tmp);
		if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		if(PlayerInfo[giveplayerid][pDie] != 2) return SendErrorMessage(playerid, MessageText[16]);
		format(string, sizeof(string), "어드민 %s님이 %s님의 기절상태를 풀어주었습니다.",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
		SendAdminMessage(string);
		format(string, sizeof(string), "어드민 %s님에 의해 기절상태에서 풀려났습니다.",PlayerInfo[playerid][pAdminName]);
		SendMessage(giveplayerid, string);
		PlayerInfo[giveplayerid][pDie] = 0;
		PlayerInfo[giveplayerid][pDieTime] = 0;
		ClearAnimations(giveplayerid);
	...
	if(strcmp(cmd, "/아이디킥", true) == 0 || strcmp(cmd, "/아킥", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/아이디킥 [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    format(string, sizeof(string), "어드민 %s님이 %s님을 강퇴하였습니다. [이유 : 아이디불량]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
		SendAdminMessage(string);
		format(string, sizeof(string), "어드민 %s님에 의해 강퇴당하셨습니다. [이유 : 아이디불량] [Ex : Hojin_Kim]", PlayerInfo[playerid][pAdminName]);
		SendMessage(giveplayerid, string);
	    KickEx(giveplayerid, 1);
	    return 1;
	}
	if(strcmp(cmd, "/킥", true) == 0 || strcmp(cmd, "/퇴장", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/킥 [플레이어번호] [이유]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/킥 플레이어번호 [이유]");
		format(string, sizeof(string), "어드민 %s님이 %s님을 강퇴하였습니다. [이유 : %s]",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), result); SendAdminMessage(string);
		format(string, sizeof(string), "어드민 %s님에 의해 강퇴당하셨습니다. [이유 : %s]", PlayerInfo[playerid][pAdminName], result); SendMessage(giveplayerid, string);
	    KickEx(giveplayerid, 1);
	    return 1;
	}
	if(strcmp(cmd, "/도움", true) == 0)
	{
	    if(NewbieChannel == 0) return SendErrorMessage(playerid, "질문 채널이 닫혀있습니다.");
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		{
	...
	if(strcmp(cmd, "/리스폰", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/리스폰 [플레이어번호]");
		new giveplayerid = strval(tmp);
		if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		format(string, sizeof(string), "어드민 %s님이 %s을 리스폰시켯습니다.",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
		SendAdminMessage(string);
		format(string, sizeof(string), "어드민 %s님에 의해 리스폰되었습니다.",PlayerInfo[playerid][pAdminName]);
		SendMessage(giveplayerid, string);
	...
	if(strcmp(cmd, "/관리자", true) == 0 || strcmp(cmd, "/관리자문의", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/관리자문의 [내용]");
		format(string, sizeof(string), "%s(%d)님의 질문 : %s",PlayerNameEx(playerid),playerid,result);
		SendAdminMessage(string, 0xA65300FF);
		format(string, sizeof(string), ""#C_GRAY"관리자 문의내용 : "#C_WHITE"%s",result);
		SendMessage(playerid, string, 0x000000FF);
		return 1;
	}
	if(strcmp(cmd, "/답변", true) == 0 || strcmp(cmd, "/답", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 1) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendErrorMessage(playerid, "/답변 [플레이어번호] [내용]");
		new giveplayerid = strval(tmp);
		if(!IsPlayerConnected(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		new length = strlen(cmdtext);
		while ((idx < length) && (cmdtext[idx] <= ' '))
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/답변 플레이어번호 [내용]");
		format(string, sizeof(string), "%s님이 %s님에게 답변 : "#C_WHITE"%s",PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid),result);
		SendAdminMessage(string);
		format(string, sizeof(string), "%s님의 답변 : "#C_WHITE"%s",PlayerInfo[playerid][pAdminName], result);
	...
	if(strcmp(cmd, "/관리업무", true) == 0 || strcmp(cmd, "/관업", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 1) return SendErrorMessage(playerid, MessageText[2]);
		if(PlayerInfo[playerid][pAdminWork] == 0)
		{
		    ...
		    new Float:x, Float:y, Float:z;
			GetPlayerPos(playerid, x, y, z);
			format(string, sizeof(string), "[관리업무]\n%s",PlayerInfo[playerid][pAdminName]);
		    ...
		    format(string, sizeof(string), "어드민 %s님이 관리업무를 시작하였습니다.", PlayerInfo[playerid][pAdminName]);
		...
		else if(PlayerInfo[playerid][pAdminWork] == 1)
		{
		    ...
		    format(string, sizeof(string), "어드민 %s님이 관리업무를 종료하였습니다.", PlayerInfo[playerid][pAdminName]);
			SendOOCNoticeMessage(string, 0x55AAAAFF);
			SetPlayerSkinEx(playerid, PlayerInfo[playerid][pClothes]);
			for(new i = 0; i < MAX_PLAYERS; i ++)
			    if(IsPlayerConnected(i))
	...
	if(strcmp(cmd, "/얼림", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/얼림 [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    format(string, sizeof(string), "어드민 %s님이 %s님을 얼렷습니다.", PlayerInfo[playerid][pAdmin], PlayerNameEx(giveplayerid)); SendAdminMessage(string);
	    format(string, sizeof(string), "어드민 %s님에 의해 얼었습니다.",PlayerInfo[playerid][pAdmin]); SendMessage(giveplayerid, string);
	    TogglePlayerControllable(giveplayerid, 0);
	    return 1;
	}
	if(strcmp(cmd, "/녹임", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/녹임 [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    format(string, sizeof(string), "어드민 %s님이 %s님을 녹였습니다.", PlayerInfo[playerid][pAdmin], PlayerNameEx(giveplayerid)); SendAdminMessage(string);
	    format(string, sizeof(string), "어드민 %s님에 의해 녹았습니다.",PlayerInfo[playerid][pAdmin]); SendMessage(giveplayerid, string);
	    TogglePlayerControllable(giveplayerid, 1);
	    return 1;
	}
	if(strcmp(cmd, "/체력", true) == 0)
	{
	    ...
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/체력 [플레이어번호] [체력]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
        tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/체력 플레이어번호 [체력]");
	    ...
	    format(string, sizeof(string), "%s님의 체력을 %f로 변경하였습니다.", PlayerNameEx(giveplayerid), health); SendMessage(playerid, string);
	    format(string, sizeof(string), "어드민 %s님에 의해 체력이 %f로 변경되었습니다.",PlayerInfo[playerid][pAdminName], health); SendMessage(giveplayerid, string);
	    return 1;
	}
	if(strcmp(cmd, "/아머", true) == 0)
	{
	    ...
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/아머 [플레이어번호] [아머]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
        tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/아머 플레이어번호 [아머]");
	    ...
	    format(string, sizeof(string), "%s님의 아머를 %f로 변경하였습니다.", PlayerNameEx(giveplayerid), armour); SendMessage(playerid, string);
	    format(string, sizeof(string), "어드민 %s님에 의해 아머가 %f로 변경되었습니다.",PlayerInfo[playerid][pAdminName], armour); SendMessage(giveplayerid, string);
	    return 1;
	}
	if(strcmp(cmd, "/서버종료", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 5) return SendErrorMessage(playerid, MessageText[2]);
	    format(string, sizeof(string), "%s님이 서버를 종료하였습니다.",PlayerInfo[playerid][pAdminName]);
	    SendOOCNoticeMessage(string);
	...
	if(strcmp(cmd, "/리붓", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 4) return SendErrorMessage(playerid, MessageText[2]);
	    format(string, sizeof(string), "%s님이 서버를 리붓하였습니다.",PlayerInfo[playerid][pAdminName]);
	    SendOOCNoticeMessage(string);
		SetTimer("RebootServer", 500, false);
		return 1;
	}
	if(strcmp(cmd, "/훈장차감", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(!IsPlayerCountryCommander(playerid)) return SendErrorMessage(playerid, MessageText[7]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/훈장수여 [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		...
		format(string, sizeof(string), "%s님의 훈장한개를 차감하였습니다.",PlayerNameEx(giveplayerid));
		SendMessage(playerid, string);
	    format(string, sizeof(string), "사령관 %s님에 의해 훈장이 한개가 반납되었습니다.",PlayerNameEx(playerid));
		SendMessage(giveplayerid, string);
		return 1;
	}
	if(strcmp(cmd, "/훈장수여", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(!IsPlayerCountryCommander(playerid)) return SendErrorMessage(playerid, MessageText[7]);
	    tmp = strtok(cmdtext, idx);
		...
		if(PlayerInfo[playerid][pCountry] != PlayerInfo[giveplayerid][pCountry]) return SendErrorMessage(playerid, MessageText[6]);
		PlayerInfo[giveplayerid][pMedal] ++;
		format(string, sizeof(string), "%s님에게 훈장한개를 수여하였습니다.",PlayerNameEx(giveplayerid));
		SendMessage(playerid, string);
	    format(string, sizeof(string), "사령관 %s님에 의해 훈장이 한개 증가하였습니다.",PlayerNameEx(playerid));
		SendMessage(giveplayerid, string);
	    return 1;
	}
	if(strcmp(cmd, "/소대확인", true) == 0 || strcmp(cmd, "/소대번호확인", true) == 0 || strcmp(cmd, "/소대번호", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
     	...
     	if(type > 0)
		{
		    ...
		    strmid(str, "번호\t소대번호\t국가\t이름", 0, 512, 512);
		    for(new i = 0; i < MAX_PLATOON; i ++)
		    {
		        if(PlatoonInfo[i][p_tCreate] == 1)
		        {
		            if(PlatoonInfo[i][p_tCountry] == 1) strmid(country, "미국", 0, 12, 12);
		            if(PlatoonInfo[i][p_tCountry] == 2) strmid(country, "러시아", 0, 12, 12);
					format(str, sizeof(str), "%s\n%d\t%d\t\t%s\t%s", str, PlatoonInfo[i][p_tNumber],PlatoonInfo[i][p_t_Number],country,PlatoonInfo[i][p_tName]);
		        }
		    }
		    ShowPlayerDialog(playerid, DIALOG_NONE, DIALOG_STYLE_LIST, "소대번호", str, "확인", "");
		    SendMessage(playerid, "소대 관련 명령어를 사용할 땐 '번호'란에 있는 번호를 사용해야 합니다. [예 : /소대장임명 /소대설정]");
		}
     	return 1;
    }
	if(strcmp(cmd, "/계급설정", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
		...
		{
		    tmp = strtok(cmdtext, idx);
		    if(!strlen(tmp)) return SendErrorMessage(playerid, "/계급설정 [플레이어번호] [계급]");
		    new giveplayerid = strval(tmp);
		    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
			if(PlayerInfo[playerid][pCountry] != PlayerInfo[giveplayerid][pCountry]) return SendErrorMessage(playerid, MessageText[6]);
		    tmp = strtok(cmdtext, idx);
		    ...
		    if(Level <= 19)
		    {
				if(type == 1)
				{
				    ...
				    {
						format(string, sizeof(string), "%s님의 계급을 %s[%d]로 설정하였습니다.",PlayerNameEx(giveplayerid),ArmyClass(Level),Level);
						SendMessage(playerid, string);
					    format(string, sizeof(string), "사령관 %s님에 의해 계급이 %s[%d]로 설정되었습니다.",PlayerNameEx(playerid),ArmyClass(Level),Level);
						SendMessage(giveplayerid, string);
				...
				{
				    format(string, sizeof(string), "어드민 %s님이 %s님의 계급을 %s[%d]로 설정하였습니다.",PlayerInfo[playerid][pAdminName],PlayerNameEx(giveplayerid),ArmyClass(Level),Level);
					SendAdminMessage(string);
				    format(string, sizeof(string), "어드민 %s님에 의해 계급이 %s[%d]로 설정되었습니다.",PlayerInfo[playerid][pAdminName],ArmyClass(Level),Level);
					SendMessage(giveplayerid, string);
					SetPlayerClass(giveplayerid, Level);
				}
			}
			else return SendErrorMessage(playerid, "계급은 0 ~ 19까지 설정 할 수 있습니다.");
		}
		else
		{
	...
	if(strcmp(cmd, "/무장해제", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    PlayerInfo[playerid][pWork] = 0;
	    ResetPlayerWeaponsEx(playerid);
	    OnPlayerCommandText(playerid, "/me 모든 무기를 버리며, 무장을 해제한다.");
	    return 1;
	}
	if(strcmp(cmd, "/비번변경", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/비번변경 [비밀번호]");
		if(4 <= strlen(tmp) < 10)
		{
	        new password[128];
			...
			OnPlayerSave(playerid);
			format(string, sizeof(string), "당신의 비밀번호가 %s로 변경 되었습니다.", tmp);
			SendMessage(playerid, string);
		}
		else return SendErrorMessage(playerid, "비밀번호는 4글자 이상, 10글지 미만이어야 합니다.");
	    return 1;
	}
	if(strcmp(cmd, "/투항", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    PlayerInfo[playerid][pWork] = 0;
	    ResetPlayerWeaponsEx(playerid);
	    ...
	    OnPlayerCommandText(playerid, "/me 모든 무기를 버리며, 투항을 한다.");
	    OnPlayerCommandText(playerid, "/손");
	    return 1;
	}
	if(strcmp(cmd, "/소대계급", true) == 0)
	{
		if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(!IsPlayerPlatoonLeader(playerid)) return SendErrorMessage(playerid, MessageText[8]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/소대초대 [플레이어번호] [계급]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    if(PlayerInfo[playerid][pCountry] != PlayerInfo[giveplayerid][pCountry]) return SendErrorMessage(playerid, MessageText[6]);
	    ...
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/소대초대 플레이어번호 [계급]");
	    new level = strval(tmp);
	    if(level > 2) return SendErrorMessage(playerid, "소대계급은 1~2까지 가능합니다.");
	    SetPlayerPlatoonLevel(playerid, PlayerInfo[playerid][pPlatoon], level);
	    format(string, sizeof(string), "소대장 %s님에 의해 %s소대 %s계급으로 변경되었습니다.",PlayerNameEx(playerid), PlatoonInfo[PlayerInfo[playerid][pPlatoon]][p_tName], PlatRankName(level));
		SendMessage(giveplayerid, string);
	    format(string, sizeof(string), "%s님을 %s소대의 %s계급으로 변경하였습니다.",PlayerNameEx(giveplayerid), PlatoonInfo[PlayerInfo[playerid][pPlatoon]][p_tName], PlatRankName(level));
		SendMessage(playerid, string);
	    return 1;
	}
	if(strcmp(cmd, "/소대초대", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(!IsPlayerPlatoonLeader(playerid)) return SendErrorMessage(playerid, MessageText[8]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/소대초대 [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		if(PlayerInfo[playerid][pCountry] != PlayerInfo[giveplayerid][pCountry]) return SendErrorMessage(playerid, MessageText[6]);
		...
		if(PlayerInfo[playerid][pPlatoonType] == 4)
	        if(PlayerInfo[giveplayerid][pCorps] != 1) return SendErrorMessage(playerid, "스나이퍼 부대 초대권한은 상대방이 전투병이어야 합니다.");
		PlayerInfo[giveplayerid][pCorps] = 4;
		SetPlayerPlatoonLevel(giveplayerid, PlayerInfo[playerid][pPlatoon], 1);
		format(string, sizeof(string), "소대장 %s님에 의해 %s소대에 가입되었습니다.",PlayerNameEx(playerid),PlatoonInfo[PlayerInfo[playerid][pPlatoon]][p_tName]);
		SendMessage(giveplayerid, string);
	    format(string, sizeof(string), "%s님을 %s소대에 초대 하였습니다.",PlayerNameEx(giveplayerid), PlatoonInfo[PlayerInfo[playerid][pPlatoon]][p_tName]);
		SendMessage(playerid, string);
		return 1;
	}
	if(strcmp(cmd, "/소대추방", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(!IsPlayerPlatoonLeader(playerid)) return SendErrorMessage(playerid, MessageText[8]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/소대추방 [플레이어번호]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		if(PlayerInfo[playerid][pCountry] != PlayerInfo[giveplayerid][pCountry]) return SendErrorMessage(playerid, MessageText[6]);
		if(PlayerInfo[playerid][pPlatoon] != PlayerInfo[giveplayerid][pPlatoon]) return SendErrorMessage(playerid, MessageText[11]);
		...
		format(string, sizeof(string), "소대장 %s님에 의해 소대에서 추방 당하였습니다.",PlayerNameEx(playerid));
		SendMessage(giveplayerid, string);
	    format(string, sizeof(string), "%s님을 소대에서 추방하였습니다.",PlayerNameEx(giveplayerid));
		SendMessage(playerid, string);
		return 1;
	}
	if(strcmp(cmd, "/공고", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(!IsPlayerCountryCommander(playerid)) return SendErrorMessage(playerid, MessageText[7]);
	    new length = strlen(cmdtext);
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/공고 [내용]");
		if(PlayerInfo[playerid][pCountry] == 1)
		{
			format(string, sizeof(string), "%s 공고 %s : "#C_WHITE"%s",CountryNameEx(PlayerInfo[playerid][pCountry]),PlayerNameEx(playerid, 2), result);
			SendCountryMessage(string, -1, HexToInt(COLOR_USA));
		}
		else if(PlayerInfo[playerid][pCountry] == 2)
		{
	...
	if(strcmp(cmd, "/팀공고", true) == 0)
	{
		...
		result[idx - offset] = EOS;
		if(!strlen(result)) return SendErrorMessage(playerid, "/팀공고 [내용]");
		if(PlayerInfo[playerid][pCountry] == 1)
		{
			format(string, sizeof(string), "%s 팀공고 %s : "#C_WHITE"%s",CountryNameEx(PlayerInfo[playerid][pCountry]), PlayerNameEx(playerid, 2), result);
	...
	if(strcmp(cmd, "/소대장임명", true) == 0)
	{
		...
		{
		    tmp = strtok(cmdtext, idx);
		    if(!strlen(tmp)) return SendErrorMessage(playerid, "/소대장임명 [플레이어번호] [소대]");
		    new giveplayerid = strval(tmp);
		    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
		    tmp = strtok(cmdtext, idx);
		    if(!strlen(tmp)) return SendErrorMessage(playerid, "/소대장임명 플레이어번호 [소대]");
		    new Platoon = strval(tmp);
		    if(PlayerInfo[giveplayerid][pCountry] != PlatoonInfo[Platoon][p_tCountry]) return SendErrorMessage(playerid, "소대와 플레이어의 국가 정보가 맞지 않습니다.");
			SetPlayerPlatoonLevel(giveplayerid, Platoon, 3);
			if(type == 1)
			{
			    format(string, sizeof(string), "사령관 %s님에 의해서 %s소대 소대장으로 임명 받았습니다.",PlayerNameEx(playerid), PlatoonInfo[Platoon][p_tName]);
				SendMessage(giveplayerid, string);
			    format(string, sizeof(string), "%s님에게 %s소대의 소대장으로 임명 하였습니다.",PlayerNameEx(giveplayerid), PlatoonInfo[Platoon][p_tName]);
				SendMessage(playerid, string);
			}
			else if(type == 2)
			{
				format(string, sizeof(string), "어드민 %s님이 %s님에게 %s국가 %s소대의 소대장으로 임명 하였습니다.", PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid),CountryNameEx(PlayerInfo[giveplayerid][pCountry]), PlatoonInfo[Platoon][p_tName]);
				SendAdminMessage(string);
			    format(string, sizeof(string), "어드민 %s님에 의해서 %s소대 소대장으로 임명 받았습니다.",PlayerInfo[playerid][pAdminName], PlatoonInfo[Platoon][p_tName]);
				SendMessage(giveplayerid, string);
			}
		}
		else
	...
	if(strcmp(cmd, "/돈주기", true) == 0)
	{
	    ...
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/돈주기 [플레이어번호] [돈]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/돈주기 플레이어번호 [돈]");
	    new Money = strval(tmp);
	    GivePlayerMoneyEx(playerid, Money);
	    format(string, sizeof(string), "어드민 %s님이 %s님의 돈을 %d$ 증가시켯습니다.", PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), Money);
		SendAdminMessage(string);
	    format(string, sizeof(string), "어드민 %s님에 의해 돈이 %d$ 증가하였습니다.",PlayerInfo[playerid][pAdminName], Money);
		SendMessage(giveplayerid, string);
	    return 1;
	}
	if(strcmp(cmd, "/저장", true) == 0 || strcmp(cmd, "/sav", true) == 0)
	{
	    ...
	    {
	        OnPlayerSave(playerid);
	    	SendMessage(playerid, "데이터 베이스를 저장하였습니다.");
	    	PlayerInfo[playerid][pSaveTimer] = 10;
		}
		else
		{
		    format(string, sizeof(string), "다시 시도해주세요. [%d초]",PlayerInfo[playerid][pSaveTimer]);
		    SendErrorMessage(playerid, string);
		}
	    return 1;
	}
	if(strcmp(cmd, "/모두저장", true) == 0)
	{
		...
		format(string, sizeof(string), "어드민 %s님이 모든 유저의 데이터를 저장하였습니다.",PlayerInfo[playerid][pAdminName]);
		SendOOCNoticeMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/소대설정", true) == 0 || strcmp(cmd, "/소대편입", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/소대설정 [플레이어번호] [소대]");
	    new giveplayerid = strval(tmp);
	    if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
	    tmp = strtok(cmdtext, idx);
	    if(!strlen(tmp)) return SendErrorMessage(playerid, "/소대설정 플레이어번호 [소대]");
	    ...
	    {
	        if(PlayerInfo[giveplayerid][pCountry] != PlatoonInfo[platoonID][p_tCountry]) return SendErrorMessage(playerid, "그 플레이어는 선택된 소대와 국가 정보가 맞지 않습니다.");
	        ...
	        format(string, sizeof(string), "어드민 %s님이 %s님의 소대를 %s[%d]로 설정하였습니다. [ID : %d]", PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), PlatoonInfo[platoonID][p_tName], PlatoonInfo[platoonID][p_t_Number], platoonID);
			SendAdminMessage(string);
	    	format(string, sizeof(string), "어드민 %s님에 의해 소대가 %s[%d]로 설정되었습니다.",PlayerInfo[playerid][pAdminName], PlatoonInfo[platoonID][p_tName], PlatoonInfo[platoonID][p_t_Number]);
			SendMessage(giveplayerid, string);
	    }
	    return 1;
	}
	if(strcmp(cmd, "/차소환", true) == 0)
	{
		...
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendMessage(playerid, "/차소환 [차번호] [색상1] [색상2]");
		ModelID = strval(tmp);
		if(ModelID < 400 || ModelID > 611) return SendErrorMessage(playerid, "차 종류는 400 ~ 611번까지 소환 할 수 있습니다.");
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendMessage(playerid, "/차소환 차번호 [색상1] [색상2]");
		color1 = strval(tmp);
		if(0 > color1 > 126) return SendMessage(playerid, "색코드는 0~126까지만 가능합니다");
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) return SendMessage(playerid, "/차소환 차번호 색상1 [색상2]");
		color2 = strval(tmp);
		if(0 > color2 > 126) return SendMessage(playerid, "색코드는 0~126까지만 가능합니다");
		GetPlayerPos(playerid, x,y,z);
	...
	if(strcmp(cmd, "/설정", true) == 0 || strcmp(cmd, "/환경설정", true) == 0 || strcmp(cmd, "/옵션", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        format(string, sizeof(string), "저사양 패치 : %s",
		SettingON_OFF(PlayerInfo[playerid][pSet_Spec]));
        ShowPlayerDialog(playerid, DIALOG_SETTING, DIALOG_STYLE_LIST, "옵션", string, "확인", "취소");
        return 1;
    }
    if(strcmp(cmd, "/help", true) == 0 || strcmp(cmd, "/도움말", true) == 0 || strcmp(cmd, "/?", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
		format(string, sizeof(string), ""#C_RED">"#C_WHITE" 서버도움말\n"#C_RED">"#C_WHITE" 명령어도움말\n"#C_RED">"#C_WHITE" 애님도움말\n"#C_RED">"#C_WHITE" 병과도움말\n"#C_RED">"#C_WHITE" 소대도움말");
        ShowPlayerDialog(playerid, DIALOG_HELP, DIALOG_STYLE_LIST, "TWWRP HELP", string, "확인","취소");
        return 1;
    }
    if(strcmp(cmd, "/미국소환", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
		...
		format(string, sizeof(string), "어드민 %s님에 의해 소환 되었습니다. [Interior : %d] [VirtualWorld : %d]",PlayerInfo[playerid][pAdminName],interior,virtualworld);
		for(new i = 0; i < MAX_PLAYERS; i ++)
		{
		...
		format(string, sizeof(string), "어드민 %s님에 의해 모든 미국 유저가 소환 되었습니다.",PlayerInfo[playerid][pAdminName]);
		SendAdminMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/러시아소환", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
		new Float:pos[3], interior = GetPlayerInterior(playerid), virtualworld = GetPlayerVirtualWorld(playerid); GetPlayerPos(playerid, pos[0], pos[1], pos[2]);
		...
		format(string, sizeof(string), "어드민 %s님에 의해 모든 러시아 유저가 소환 되었습니다.",PlayerInfo[playerid][pAdminName]);
		SendAdminMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/미국체력", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    format(string, sizeof(string), "어드민 %s님에 의해 체력이 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		for(new i = 0; i < MAX_PLAYERS; i ++)
		{
		    if(IsPlayerConnected(i) && IsPlayerLogin(i))
		    {
		...
		format(string, sizeof(string), "어드민 %s님에 의해 모든 미국 유저의 체력이 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		SendAdminMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/러시아체력", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    format(string, sizeof(string), "어드민 %s님에 의해 체력이 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		...
		format(string, sizeof(string), "어드민 %s님에 의해 모든 러시아 유저의 체력이 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		SendAdminMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/모두체력", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    format(string, sizeof(string), "어드민 %s님에 의해 체력이 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		...
		format(string, sizeof(string), "어드민 %s님에 의해 모든 유저의 체력이 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		SendAdminMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/모두아머", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    format(string, sizeof(string), "어드민 %s님에 의해 아머가 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		for(new i = 0; i < MAX_PLAYERS; i ++)
		{
		    if(IsPlayerConnected(i) && IsPlayerLogin(i))
		    {
		...
		format(string, sizeof(string), "어드민 %s님에 의해 모든 유저의 아머가 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		SendAdminMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/미국아머", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    format(string, sizeof(string), "어드민 %s님에 의해 아머가 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		...
		format(string, sizeof(string), "어드민 %s님에 의해 모든 미국 유저의 아머가 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		SendAdminMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/러시아아머", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
	    format(string, sizeof(string), "어드민 %s님에 의해 아머가 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		...
		format(string, sizeof(string), "어드민 %s님에 의해 모든 러시아 유저의 아머가 100.0으로 변경 되었습니다.",PlayerInfo[playerid][pAdminName]);
		SendAdminMessage(string);
	    return 1;
	}
	if(strcmp(cmd, "/모두소환", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, MessageText[2]);
		new Float:pos[3], interior = GetPlayerInterior(playerid), virtualworld = GetPlayerVirtualWorld(playerid); GetPlayerPos(playerid, pos[0], pos[1], pos[2]);
		...
		format(string, sizeof(string), "어드민 %s님에 의해 모든 유저가 소환 되었습니다.");
		SendAdminMessage(string);
	    return 1;
	}
    if(strcmp(cmd, "/adminhelp", true) == 0 || strcmp(cmd, "/ah", true) == 0 || strcmp(cmd, "/어드민도움말", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        if(PlayerInfo[playerid][pAdmin] < 0) return SendErrorMessage(playerid, MessageText[2]);
        if(PlayerInfo[playerid][pAdmin] >= 1) SendMessage(playerid, "Admin Help Lv.1) /답변 /admin /관리업무 /출두 /소환 /콜 /드모르간 /드모르간확인",COLOR_ADMIN);
        if(PlayerInfo[playerid][pAdmin] >= 2) SendMessage(playerid, "Admin Help Lv.2) /위성 /위성끄기 /ip /킥 /얼림 /녹임 /모두저장 /텔레포트 /리스폰 /공지 /경고 /경고차감 /총검사 /아이디킥",COLOR_ADMIN);
        if(PlayerInfo[playerid][pAdmin] >= 2) SendMessage(playerid, "Admin Help Lv.2) /채팅금지 /스킨변경 /차리스폰 /모든차리스폰 /미국소환 /러시아소환 /모두체력 /모두아머",COLOR_ADMIN);
        if(PlayerInfo[playerid][pAdmin] >= 3) SendMessage(playerid, "Admin Help Lv.3) /체력 /아머 /차소환 /돈주기 /계급설정 /소대설정 /좌표이동 /무기 /인테리어 /버추얼월드 /정보확인 /질문채널",COLOR_ADMIN);
        if(PlayerInfo[playerid][pAdmin] >= 3) SendMessage(playerid, "Admin Help Lv.3) /귓말검사 /모든차기름 /점령가동 /아이디밴 /영구밴 /밴해제 /관리자응급",COLOR_ADMIN);
        if(PlayerInfo[playerid][pAdmin] >= 4) SendMessage(playerid, "Admin Help Lv.4) /점령지관리 /갱존관리 /건물관리 /리붓 /리더 /팩션관리 /소대장임명 /ip밴 /ip해제 /광역밴 /소대번호 /상품관리 /시동관리",COLOR_ADMIN);
        if(PlayerInfo[playerid][pAdmin] >= 5) SendMessage(playerid, "Admin Help Lv.5) /서버종료 /어드민 /어드민이름 /관리자레벨 /페이데이",COLOR_ADMIN);
        return 1;
    }
    if(strcmp(cmd, "/시동관리", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        if(PlayerInfo[playerid][pAdmin] < 4) return SendErrorMessage(playerid, MessageText[2]);
		if(!IsPlayerInAnyVehicle(playerid)) return SendErrorMessage(playerid, "차량에 탑승 후 다시 시도해주세요.");
		new ModelID = GetVehicleModel(GetPlayerVehicleID(playerid));
		//ModelID = 0;
		if(VehiclestartupInfo[ModelID][v_sCreate] == 0)
		{
			PlayerInfo[playerid][pVehicleStart_Cre] = 0;
			PlayerInfo[playerid][pVehicleStart_Model] = ModelID;
			format(string, sizeof(string), "\t\t"#C_GRAY"- 시동 권한 - \n\n"#C_WHITE"%s차량의 시동 권한이 없는 상태입니다.\n'추가'버튼을 눌러 시동 권한을 추가해보세요.");
		    ShowPlayerDialog(playerid, DIALOG_VEHICLE_STARTUP, DIALOG_STYLE_MSGBOX, "시동관리", string, "추가", "취소");
		}
		else if(VehiclestartupInfo[ModelID][v_sCreate] == 1)
		{
            format(string, sizeof(string), "소대 : %s\n계급 : %s",GetVehicleStartupInfo(ModelID, 1), GetVehicleStartupInfo(ModelID, 2));
            ShowPlayerDialog(playerid, DIALOG_VEHICLE_STARTUP, DIALOG_STYLE_LIST, "시동관리", string, "확인", "이전");
            PlayerInfo[playerid][pVehicleStart_Cre] = 1;
		}
        return 1;
    }
    if(strcmp(cmd, "/소대관리", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        if(PlayerInfo[playerid][pAdmin] < 4) return SendErrorMessage(playerid, MessageText[2]);
        PlayerInfo[playerid][pPlatoonCre] = 0;
        ShowPlayerDialog(playerid, DIALOG_PLATOON, DIALOG_STYLE_LIST, "팩션 관리", ""#C_RED"> "#C_WHITE"소대생성\n"#C_RED"> "#C_WHITE"소대관리\n"#C_RED"> "#C_WHITE"소대삭제", "확인","취소");
        return 1;
    }
    if(strcmp(cmd, "/점령지관리", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        if(PlayerInfo[playerid][pAdmin] < 4) return SendErrorMessage(playerid, MessageText[2]);
        PlayerInfo[playerid][pOccupyAreaCre] = 0; PlayerInfo[playerid][pOccupyAreaEditType] = 0;
        format(string, sizeof(string), ""#C_RED"> "#C_WHITE"점령지 생성\n"#C_RED"> "#C_WHITE"점령지 편집\n"#C_RED"> "#C_WHITE"점령지 삭제");
        ShowPlayerDialog(playerid, DIALOG_OCCUPY_AREA, DIALOG_STYLE_LIST, "점령지 관리", string, "확인","취소");
        return 1;
    }
    if(strcmp(cmd, "/갱존관리", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        if(PlayerInfo[playerid][pAdmin] < 4) return SendErrorMessage(playerid, MessageText[2]);
        PlayerInfo[playerid][pGangzoneNameEdit] = 0;
        if(!PlayerInfo[playerid][pGangzoneCre]) format(string, sizeof(string), ""#C_RED"> "#C_WHITE"갱존생성\n"#C_RED"> "#C_WHITE"갱존편집\n"#C_RED"> "#C_WHITE"갱존삭제");
        else if(0 < PlayerInfo[playerid][pGangzoneCre] <= 2) format(string, sizeof(string), ""#C_RED"> "#C_WHITE"확정\n"#C_RED"> "#C_WHITE"갱존생성 취소");
        ShowPlayerDialog(playerid, DIALOG_GANGZONE, DIALOG_STYLE_LIST, "갱존관리", string, "확인","취소");
        return 1;
    }
    if(strcmp(cmd, "/건물관리", true) == 0)
    {
        ...
        if(PlayerInfo[playerid][pCreEnterID] == -1) format(string, sizeof(string), ""#C_RED"> "#C_WHITE"건물생성\n"#C_RED"> "#C_WHITE"건물편집\n"#C_RED"> "#C_WHITE"건물삭제");
        else format(string, sizeof(string), ""#C_RED"> "#C_WHITE"건물 내부지정\n"#C_RED"> "#C_WHITE"건물생성 취소");
        ShowPlayerDialog(playerid, DIALOG_BUILD, DIALOG_STYLE_LIST, "건물관리", string, "확인","취소");
        return 1;
    }
    if(strcmp(cmd, "/상품관리", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        if(PlayerInfo[playerid][pAdmin] < 4) return SendErrorMessage(playerid, MessageText[2]);
        PlayerInfo[playerid][pProductEdit] = 0;
        format(string, sizeof(string), ""#C_RED"> "#C_WHITE"상품생성\n"#C_RED"> "#C_WHITE"상품편집\n"#C_RED"> "#C_WHITE"상품삭제");
        ShowPlayerDialog(playerid, DIALOG_PRODUCT, DIALOG_STYLE_LIST, "상품관리", string, "확인","취소");
        return 1;
    }
    if(strcmp(cmd, "/내정보", true) == 0 || strcmp(cmd, "/스탯", true) == 0 || strcmp(cmd, "/군번줄", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        My_Information(playerid, playerid);
        OnPlayerCommandText(playerid, "/me 목에 걸려있던 군번줄을 확인한다.");
        return 1;
    }
    if(strcmp(cmd, "/군번보", true) == 0)
    {
        if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
        tmp = strtok(cmdtext, idx);
        if(!strlen(tmp)) return SendErrorMessage(playerid, "/군번보 [플레이어번호]");
		new giveplayerid = strval(tmp);
		if(!IsPlayerConnected(giveplayerid) && !IsPlayerLogin(giveplayerid)) return SendErrorMessage(playerid, MessageText[5]);
        ...
        format(string, sizeof(string), "/me %s에게 군번줄을 보여줍니다.", PlayerNameEx(giveplayerid));
        OnPlayerCommandText(playerid, string);
        return 1;
    }
	if(strcmp(cmd, "/좌표이동", true) == 0)
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
     	if(PlayerInfo[playerid][pAdmin] < 3) return SendErrorMessage(playerid, MessageText[2]);
	...
	if(strcmp(cmd, "/출두", true) == 0 || strcmp(cmd, "/가", true) == 0 || strcmp(cmd, "/가자", true) == 0)
	{
		...
		format(string, sizeof(string), "%s님에게 텔레포트 되었습니다. [INT : %d VirtualWorld : %d]",PlayerNameEx(giveplayerid), interior, virtualworld);
		SendMessage(playerid, string);
        if(IsPlayerInAnyVehicle(playerid))
        {
            VehicleID = GetPlayerVehicleID(playerid);
	...
	if(strcmp(cmd, "/소환", true) == 0 || strcmp(cmd, "/소", true) == 0)
	{
		...
		format(string, sizeof(string), "%s님에게 텔레포트 되었습니다. [INT : %d VirtualWorld : %d]",PlayerNameEx(playerid), interior, virtualworld);
		SendMessage(giveplayerid, string);
        if(IsPlayerInAnyVehicle(playerid))
        {
	...
	{
	    if(!IsPlayerLogin(playerid)) return SendErrorMessage(playerid, MessageText[0]);
	    if(PlayerInfo[playerid][pBattalionRadio] == 1) return SendErrorMessage(playerid, "무전기 사용중에는 사용 할 수 없습니다.");
	    ApplyAnimation(playerid,"PED","facgum",4.1,0,1,1,1,1);
	    return 1;
	}
	else
	{
		format(string, sizeof(string), "없는 명령어 입니다. [%s]", cmdtext);
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    if(IsPlayerLogin(playerid))
    {
        if(newkeys == KEY_ANALOG_LEFT)
        {
            ...
            if(!IsPlayerInAnyVehicle(playerid))
            {
            	if(PlayerInfo[playerid][pBattalionRadio] == 0)
            	{
            	    ...
            	    SendMessage(playerid, "대대 무전기를 켯습니다.");
            	...
            	else if(PlayerInfo[playerid][pBattalionRadio] == 1)
            	{
            	    ...
            	    SendMessage(playerid, "대대 무전기를 종료하였습니다.");
            	}
			}
        }
        if(newkeys & KEY_YES)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_INVENTORY, DIALOG_STYLE_LIST, "가방", string, "확인", "취소");
        }
        if(newkeys & KEY_FIRE)
        {
        ...
        if(newkeys == KEY_SECONDARY_ATTACK)
        {
		    ...
		    {
		        OnPlayerCommandText(playerid, "/me 피우던 담배를 땅바닥에 버리고 불씨를 제거하기 위해 발로 밟는다.");
		    ...
		    else
		    {
		        if(PlayerInfo[playerid][pCountry] == 1)
			    {
	            	...
	            	{
						new string[256];
						new Message0[] = ""#C_GRAY"______________________________미국 임시소대 가입______________________________"#C_WHITE"\n\n";
						new Message1[] = "임시소대란 아직 소대가 정해지지 않은 군인들을 모아둔 소대로써\n";
						new Message2[] = "이 소대에서 군대RP를 알아가고, 국가가 어떻게 돌아가는지를 파악할 수 있습니다.\n";
						format(string, sizeof(string), "%s%s%s",Message0,Message1,Message2);
	            	    ShowPlayerDialog(playerid, DIALOG_TEMP_PLATOON, DIALOG_STYLE_MSGBOX, "임시소대 가입", string, "가입", "취소");
	            	}
	            }
	            if(PlayerInfo[playerid][pCountry] == 2)
			    {
	            	...
	            	{
	            	    new string[256];
						new Message0[] = ""#C_GRAY"______________________미국 임시소대 가입______________________"#C_WHITE"\n\n";
						new Message1[] = "임시소대란 아직 소대가 정해지지 않은 군인들을 모아둔 소대로써\n";
						new Message2[] = "이 소대에서 군대RP를 알아가고, 국가가 어떻게 돌아가는지를 파악할 수 있습니다.\n";
						format(string, sizeof(string), "%s%s%s",Message0,Message1,Message2);
	            	    ShowPlayerDialog(playerid, DIALOG_TEMP_PLATOON, DIALOG_STYLE_MSGBOX, "임시소대 가입", string, "가입", "취소");
				...
				for(new p_d = 0; p_d < MAX_PRODUCT; p_d ++)
				{
				    if(IsPlayerInRangeOfPoint(playerid, 1.0, ProductInfo[p_d][p_dPos][0], ProductInfo[p_d][p_dPos][1], ProductInfo[p_d][p_dPos][2]))
				    {
				        ...
				        for(new i = 0, l = sizeof(ItemInfo); i < l; i ++)
					    {
					    	...
					    	else if(i == l -1) return SendErrorMessage(playerid, "상품 생성 오류가 발생 하였습니다. 오류코드 : 1001");
						}
				        format(string, sizeof(string), ""#C_GRAY"- %s 구매 - "#C_WHITE"\n이름 : %s\n가격 : %d$\n최대 소지 가능 갯수 : %d개\n",
				        ...
				        ShowPlayerDialog(playerid, DIALOG_PRODUCT_BUY, DIALOG_STYLE_INPUT, "상품구입", string, "구매","취소");
				        break;
				    }
				}
	            for(new e = 0; e < MAX_ENTER; e ++)
			  	...
			  	for(new i = 0; i < MAX_MOVE_OBJECT; i ++)
				{
	                if(MoveObjectInfo[i][mCreate] == 1)
	                {
	                    if(IsPlayerInRangeOfPoint(playerid, MoveObjectInfo[i][mDistance], MoveObjectInfo[i][mPosX],MoveObjectInfo[i][mPosY],MoveObjectInfo[i][mPosZ]))
	                    {
							...
							{
							    new string[128];
							    format(string, sizeof(string), "다시 시도해주세요. [%d초 남음]", MoveObjectTimer[i]);
							    SendErrorMessage(playerid, string);
							}
							return 1;
	                    }
				...
				if(IsPlayerInRangeOfPoint(playerid, 2.0,1214.6433,-251.8133,1021.7993))
				{
			        ...
			        {
			            new string[512];
						new Message0[] = "\t"#C_GRAY"The World War Role Play에 오신 것을 환영합니다.\t\t\n\n"#C_WHITE"";
						new Message1[] = "아직 기초 훈련이 되어있지 않습니다.\n";
						new Message2[] = "기초 훈련을 받으시고 싶으시다면 '확인'을 눌러주세요.\n\n";
						format(string, sizeof(string), "%s%s%s",Message0,Message1,Message2);
						ShowPlayerDialog(playerid, DIALOG_BASIC_TRAINING, DIALOG_STYLE_MSGBOX, "기초훈련", string, "확인", "취소");
			        }
			        else if(PlayerInfo[playerid][pNPCQwest] == 6)
					{
					    ShowPlayerDialog(playerid, DIALOG_BASIC_TRAINING, DIALOG_STYLE_LIST, "병과선택", "전투병\n공병\n의무병", "선택","");
					}
				}
			    else if(IsPlayerInRangeOfPoint(playerid, 2.0,1213.0793,-246.6340,1021.7993))
			    {
			        if(PlayerInfo[playerid][pNPCQwest] == 2 || PlayerInfo[playerid][pNPCQwest] == 5)
			        {
						...
						ShowPlayerDialog(playerid, DIALOG_BASIC_TRAINING, DIALOG_STYLE_LIST, "기초훈련 제 1장", "AK47/M4 돌격병\nMP5 의무병\nMP5 공병", "확인", "취소");
			        }
			    }if(IsPlayerInRangeOfPoint(playerid, 2.0, 296.4462,-38.3054,1001.5156)) //미국 무장소
			    {
			        if(PlayerInfo[playerid][pCountry] == 1)
			        {
			            if(PlayerInfo[playerid][pPlatoon] == 0) return SendErrorMessage(playerid, "소대가입이 되어있지 않습니다.");
						new string[128];
						format(string, sizeof(string), ""#C_RED"> "#C_WHITE"무장\n"#C_RED"> "#C_WHITE"환복");
			            ShowPlayerDialog(playerid, DIALOG_ARME, DIALOG_STYLE_LIST, "무장소", string, "확인", "취소");
			        }
			    }
			    if(IsPlayerInRangeOfPoint(playerid, 2.0, 295.4839,-80.6940,1001.5156)) //러시아 무장소
			    {
			        if(PlayerInfo[playerid][pCountry] == 2)
			        {
			            if(PlayerInfo[playerid][pPlatoon] == 0) return SendErrorMessage(playerid, "소대가입이 되어있지 않습니다.");
		...
		if(newkeys == KEY_CTRL_BACK)
		{
		    if(!IsPlayerInAnyVehicle(playerid))
		    {
				...
				if(Number == 1) // 버려진 아이템이 한 개 일 때
				{
				    ...
				    new string[128];
				    format(string, sizeof(string), "/me %s를 줍는다. [%d개]", DropItemInfo[dropid][iName], number);
				    OnPlayerCommandText(playerid, string);
				    GivePlayerItemEx(playerid, itemid, number);
	                DeleteDropItem(dropid, number);
				}
				else if(Number > 1) // 버려진 아이템이 두 개 이상일 때
				{
				    new string[256];
					format(string, sizeof(string), ""#"{E0D674}""모두 회수하기\n\n"#C_GRAY"Number\tItem Name");
					for(new DropItemID = 0; DropItemID < MAX_DROP_ITEM; DropItemID ++)
					{
				    ...
				    ShowPlayerDialog(playerid, DIALOG_TAKE_WEAPON, DIALOG_STYLE_LIST, "무기회수", string, "확인", "취소");
				}
			}
		}
		        ...
		        if(PlayerInfo[playerid][pFirstaid] == 1)
		        {
		            if(IsPlayerLogin(PlayerInfo[playerid][pgFirstaid]) && PlayerInfo[PlayerInfo[playerid][pgFirstaid]][pDie] == 2)
		            {
			            ...
			            if(PlayerInfo[playerid][pDoingGage] >= 100)
			            {
			                ...
			                HidePlayerProgressBar(playerid, DoingBar[playerid]);
			                OnPlayerCommandText(PlayerInfo[playerid][pgFirstaid], "/정상");
			                OnPlayerCommandText(PlayerInfo[playerid][pgFirstaid], "/me 응급처치를 받고 일어난다.");
			                format(string, sizeof(string), "%s님에 의해 기절 상태에서 풀려났습니다.",PlayerNameEx(playerid));
			                SendMessage(PlayerInfo[playerid][pgFirstaid], string);
			            }
					}
					else
					{
					    SendMessage(playerid, "상대방이 게임을 나갔거나, 기절 상태가 아닙니다.");
					}
		        }
			        	    ...
			        	    if(IsPlayerInRangeOfPoint(playerid, 3.0, Occupy_AreaInfo[o][oPosX], Occupy_AreaInfo[o][oPosY], Occupy_AreaInfo[o][oPosZ]))
			        	    {
				        	    if(Occupy_AreaInfo[o][oCountry] != PlayerInfo[playerid][pCountry])
				        	    {
				        	        if(Occupy_AreaInfo[o][oOccupy] == 1)
				        	    	{
					        	        ...
					        	        if(g == -1) return SendErrorMessage(playerid, "이 점령지 주변에 갱존이 없어 점령하지 못 합니다.");
									...
									else return SendErrorMessage(playerid, "점령이 차단된 지역입니다.");
				        	    }
							}
						}
					}
...
public OnVehicleDamageStatusUpdate(vehicleid, playerid)
{
	...
	if(health < 300.0)
	{
	    ...
	    VehicleAct(vehlcieid, "엔진이 파열 되어, 시동이 자동적으로 종료되었다.");
	}
	return 1;
}
...
public OnPlayerClickPlayer(playerid, clickedplayerid, source)
{
	if(PlayerInfo[playerid][pAdmin] >= 4)
	{
	    if(IsPlayerLogin(clickedplayerid))
	    {
	    	...
	    	ShowPlayerDialog(playerid, DIALOG_PLAYERCLICK, DIALOG_STYLE_LIST, PlayerNameEx(clickedplayerid), "출두\n소환\n정보수정", "확인", "취소");
		}
	}
	return 1;
}
    ...
    if(IsPlayerLogin(playerid) && PlayerInfo[playerid][pDoing] == 1)
    {
        if(IsPlayerInRangeOfPoint(playerid, 3.0, Occupy_AreaInfo[OccupyID][oPosX], Occupy_AreaInfo[OccupyID][oPosY], Occupy_AreaInfo[OccupyID][oPosZ]) && Occupy_AreaInfo[OccupyID][oOccupy] == 1)
		{
			...
			else
			{
			    ...
			    format(string, sizeof(string), "%s국가가 %s점령지를 점령하였습니다. [%d년-%d월-%d일 %d시%d분]",
	    ...
	    */
	    ...
	    if(PlayerInfo[playerid][pNPCQwest] == 4)
	    {
		    if(IsPlayerInRangeOfPoint(playerid, 2.0,QwestInfo[playerid][qPosX],QwestInfo[playerid][qPosY],QwestInfo[playerid][qPosZ]))
		    {
		        ...
		        SendMessage(playerid, "당신이 선택한 무기로 30초간 연습할 수 있습니다.");
		    }
		}
	}
	return 1;
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_REGISTER)
	{
		...
		if(response)
		{
			...
			{
			    format(string,sizeof(string), ""#C_GRAY"\tThe World War Role Play Game\n\n"#C_WHITE"당신의 데이터베이스가 발견되지 않았습니다.\n원하시는 비밀번호를 입력하고 '회원가입'을 클릭해주세요.\n\n"#C_SOFTRED"접속한 IP : %s",playerip);
		    	ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD,"회원가입",string,"회원가입","");
			    SendErrorMessage(playerid, "비밀번호는 4글자 이상, 10글지 미만이어야 합니다.");
			}
		}
		else
		{
		    format(string,sizeof(string), ""#C_GRAY"\tThe World War Role Play Game\n\n"#C_WHITE"당신의 데이터베이스가 발견되지 않았습니다.\n원하시는 비밀번호를 입력하고 '회원가입'을 클릭해주세요.\n\n"#C_SOFTRED"접속한 IP : %s",playerip);
		    ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD,"회원가입",string,"회원가입","");
		    SendErrorMessage(playerid, "다시 입력하세요.");
		}
	    return 1;
	}
	if(dialogid == DIALOG_LOGIN)
	{
		...
		if(response)
		{
		    if(!strlen(inputtext))
		    {
		        format(string,sizeof(string), ""#C_GRAY"\tThe World War Role Play Game\n\n"#C_WHITE"당신의 데이터베이스가 발견 되었습니다.\n비밀번호를 입력하고 '로그인'을 클릭해주세요.\n\n"#C_SOFTRED"접속한 IP : %s",playerip);
			    ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD,"로그인",string,"로그인","");
			    SendErrorMessage(playerid, "다시 입력하세요.");
			    return 1;
		    }
		    new query[256],
				password[128];
		    SHA512(inputtext, password, 128);
			strmid(password, password, 0, 128, 128); // 정확히 128-bit만 추출
			...
			else
			{
			    format(string,sizeof(string), ""#C_GRAY"\tThe World War Role Play Game\n\n"#C_WHITE"당신의 데이터베이스가 발견 되었습니다.\n비밀번호를 입력하고 '로그인'을 클릭해주세요.\n\n"#C_SOFTRED"접속한 IP : %s",playerip);
		    	ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD,"로그인",string,"로그인","");
			    SendErrorMessage(playerid, "비밀번호가 맞지 않습니다.");
			}
			cache_delete(result);
		}
		else
	...
	if(dialogid == DIALOG_HELP)
	{
	    if(response)
	    {
	        if(listitem == 0)
	        {
				...
				ShowPlayerDialog(playerid, DIALOG_SERVER_INFO, DIALOG_STYLE_MSGBOX, "TWWRP INFO", string, "확인", "이전");
			}
	        if(listitem == 1)
	        {
	            ...
	            SendMessage(playerid, "기본 명령어 - /군번줄 /군번보 /도움말 /질문 /도움 /관리자 /환경설정 /탄차 /하사관접속 /장교접속 /기부 /기부금액 /탄차");
	            SendMessage(playerid, "기본 명령어 - /저장 /비번변경");
	            SendMessage(playerid, "채팅 명령어 - /s /c /me /st /so /확성기");
	            SendMessage(playerid, "전투 명령어 - /모르핀 /확인사살 /총버리기 /비상무장 /폭탄설치 /폭파 /점령지현황 /무장해제 /투항");
	        }
	        if(listitem == 2)
	        {
	            SendMessage(playerid, "____________________________");
	            SendMessage(playerid, "/앉기 /충성");
	        }
	        if(listitem == 3) //병과도움말
	        {
	            if(PlayerInfo[playerid][pCorps] == 0) return SendErrorMessage(playerid, "당신은 아직 병과가 없습니다.");
				else if(PlayerInfo[playerid][pCorps] == 1)
				{
				    SendMessage(playerid, "_______________________________________");
					SendMessage(playerid, "전투병 : /쌀포대 /방탄복 /자가방탄복");
				}
				else if(PlayerInfo[playerid][pCorps] == 2)
				{
				    SendMessage(playerid, "_________________________________________________");
				    SendMessage(playerid, "공병 : /바리[1-3] /스파이크 /기름충전 /차량수리");
				}
				else if(PlayerInfo[playerid][pCorps] == 3)
				{
				    SendMessage(playerid, "___________________________");
				    SendMessage(playerid, "의무병 : /응급 /치료 /보급");
				}
	        ...
	        if(listitem == 4) //소대도움말
	        {
	            ...
	            if(IsPlayerPlatoonLeader(playerid))
					SendMessage(playerid, "소대장 명령어 : /소대초대 /소대추방 /소대계급 /모닥불");
	            SendMessage(playerid, "소대 명령어 : /소대원접 /bd /소대탈퇴");
	        }
	    }
	    else
	    {
	...
	if(dialogid == DIALOG_TELEPORT)
	{
	    if(response)
	    {
			if(PlayerInfo[playerid][pTeleport] == -1)
			{
		        ...
		        {
		            PlayerInfo[playerid][pTeleport] = 1;
		            ShowPlayerDialog(playerid, DIALOG_TELEPORT, DIALOG_STYLE_LIST, "국가이동", "미국\n러시아", "이동", "이전");
		        }
		        if(listitem == 1)
		        {
					...
					ShowPlayerDialog(playerid, DIALOG_TELEPORT, DIALOG_STYLE_LIST, "점령지 이동", string, "이동", "이전");
		        }
			}
			else
			{
			    if(PlayerInfo[playerid][pTeleport] == 1)
			 	{
			 	    if(listitem == 0)
				    {
			            ...
			            SetPlayerFacingAngle(playerid, 0.0);
			            SendMessage(playerid, "미국으로 이동하였습니다.");
				    }
				    if(listitem == 1)
				    {
        				SetPlayerPosEx(playerid, -1289.2861,443.1602,7.1809, 0, 0);
			            SetPlayerFacingAngle(playerid, 0.0);
			            SendMessage(playerid, "러시아로 이동하였습니다.");
				    }
				    return 1;
				}
				else if(PlayerInfo[playerid][pTeleport] == 2)
			 	{
			 	    ...
			 	    else
			 	    {
	                    ...
	                    SetPlayerFacingAngle(playerid, 0.0);
	                    format(string, sizeof(string), "점령지 %s로 이동하였습니다.",Occupy_AreaInfo[OccupyID][oName]);
	                    SendMessage(playerid, string);
	                }
			 	    return 1;
			 	}
	    ...
	    {
	        if(PlayerInfo[playerid][pTeleport] == 1)
                OnPlayerCommandText(playerid, "/텔레포트");
	        else if(PlayerInfo[playerid][pTeleport] == 2)
                OnPlayerCommandText(playerid, "/텔레포트");
	    }
	    return 1;
	...
	if(dialogid == DIALOG_SEX)
	{
	    if(response)
	    {
	        if(listitem == 0)
	        {
	            ...
	            SendMessage(playerid, "당신의 성별은 남성입니다, 당신의 나이를 정해주세요.");
	        }
	        if(listitem == 1)
	        {
	            ClearChat(playerid, 20);
	            PlayerInfo[playerid][pSex] = 2;
	            SendMessage(playerid, "당신의 성별은 여성입니다, 당신의 나이를 정해주세요.");
	        }
			new string[256]; strmid(string, "나이는 19세부터 60세까지 정하실 수 있습니다."#C_WHITE"", 0, 256, 256);
			for(new i = 19; i <= 60; i ++)
			{
				format(string, sizeof(string), "%s\n%d세",string, i);
			}
	        ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_LIST, "나이", string, "확인", "이전");
	    }
	    else
	    {
	        ClearChat(playerid, 20);
	...
	if(dialogid == DIALOG_AGE)
	{
		if(response)
		{
			...
			format(string, sizeof(string), "당신의 나이는 %d세 입니다, 당신의 국적을 정해주세요.", PlayerAge);
			SendMessage(playerid, string);
			ShowPlayerDialog(playerid, DIALOG_COUNTRY, DIALOG_STYLE_LIST, "국적", "미국\n러시아", "확인", "이전");
		}
		else
		{
		    ClearChat(playerid, 20);
	...
	if(dialogid == DIALOG_COUNTRY)
	{
	    if(response)
	    {
	        if(listitem == 0)
	        {
	            ...
	            SendMessage(playerid, "당신의 국적은 미국입니다.");
	        }
	        if(listitem == 1)
	        {
	            ClearChat(playerid, 20);
	            PlayerInfo[playerid][pCountry] = 2;
	            SendMessage(playerid, "당신의 국적은 러시아입니다.");
	        }
	    ...
	    {
	        ClearChat(playerid, 20);
	        SendMessage(playerid, "당신의 나이를 정해주세요.");
	        new string[128]; strmid(string, ""#C_TUTCOLOR"나이는 19세부터 60세까지 정하실 수 있습니다."#C_WHITE"", 0, 256, 256);
			for(new i = 19; i <= 60; i ++)
			{
				format(string, sizeof(string), "%s\n%d세",string, i);
			}
	...
	if(dialogid == DIALOG_BASIC_TRAINING)
	{
	    if(response)
	    {
			...
			{
			    new string[256];
			    format(string, sizeof(string), "\t\t\t"#C_GRAY"The World War Role Play - 기초훈련 제 1장\t\t\n\n"#C_WHITE"당신이 원하는 병과를 고르기 전 미리 실전 테스트를 하여, 원하는 병과로 지원할 수 있습니다.\n체크포인트에 가서 'F'키를 눌러주신 후 원하는 무기를 선택해보세요.\n\n");
			    ShowPlayerDialog(playerid, DIALOG_BASIC_TRAINING, DIALOG_STYLE_MSGBOX, "기초훈련 제 1장", string, "확인", "이전");
			    PlayerInfo[playerid][pNPCQwest] = 1;
			}
			else if(PlayerInfo[playerid][pNPCQwest] == 1)
			{
			    PlayerInfo[playerid][pNPCQwest] = 2;
			    SendMessage(playerid, "뒷쪽 픽업으로 이동하여 F키를 눌러주세요.");
			}
			else if(PlayerInfo[playerid][pNPCQwest] == 3)
			{
			    ...
			    SendMessage(playerid, "뒤에있는 사격장으로 이동해주세요.");
			}
			...
			else if(PlayerInfo[playerid][pNPCQwest] == 6)
			{
			    ...
			    new string[1024];
				new Message0[] = "\t\t\t\t"#C_GRAY"The World War Role Play - 기초훈련 제 2장\t\t\n\n"#C_WHITE"";
				new Message1[] = "Role Playing\n실제세상과 다른 또 하나의 나를 가상세계에서 살아가며, 가상세계의 나의 역할에 대해 책임감을 갖는 것 입니다.\n";
				new Message2[] = "In Character\n가상세계에 있는 또 다른 자아, 가상세계의 규칙에 맞게 살아가는 캐릭터다.\n";
				new Message3[] = "Out Of Character\nIC상의 캐릭터 밖에 있는 진짜 내가 살아가고 있는 세계를 말합니다.\n";
				new Message4[] = "파워게이밍\n행동 명령어로 말도 안 되는 것을 적어 사용하는 것을 말합니다.\n";
				new Message5[] = "메타게이밍\n가상세계인 IC에서 실제로는 발음할 수 없는 단어나 문자를 사용하는 것을 말하며,'ㅋㅋ', 'ㄴㄴ', '뷁뷁'등을 가르킵니다.\n";
				new Message6[] = "NonRP\n현실세계에서는 말도 안 되는 행동을 하는 행위를 말합니다. 예) 버니합핑, 해상도주 등\n";
				new Message7[] = "BadRP\n도덕적으로 문제가 되는 RP를 하는 것을 말합니다.\n\n";
				new Message8[] = "제가 알려드리는 것은 여기까지 입니다. 이제 IC세계로 이동해야 합니다. 지금 이동하시려면 '이동'을 클릭해주세요.\n";
			    ...
			    ShowPlayerDialog(playerid, DIALOG_BASIC_TRAINING, DIALOG_STYLE_MSGBOX, "기초훈련 제 2장", string, "베이스이동","이전");
			}
	...
	if(dialogid == DIALOG_BUILD)
	{
	    if(response)
	    {
	        if(PlayerInfo[playerid][pAdmin] < 4) return SendErrorMessage(playerid, "어드민 레벨이 부족하여 작업이 취소되었습니다.");
	        if(PlayerInfo[playerid][pCreEnterID] != -1 && PlayerInfo[playerid][pCreEnterName] == 1)
	        {
			    ...
			    ShowPlayerDialog(playerid, DIALOG_BUILD, DIALOG_STYLE_INPUT, "건물생성", string, "확정","이전");
				return 1;
	        }
				...
				if(strlen(inputtext))
				{
					...
					PlayerInfo[playerid][pCreEnterPos2][2],PlayerInfo[playerid][pCreEnterVW1],PlayerInfo[playerid][pCreEnterInt1],PlayerInfo[playerid][pCreEnterVW2],
				 	PlayerInfo[playerid][pCreEnterInt2],PlayerInfo[playerid][pCreEnterType],inputtext) == 1)
				 	    SendMessage(playerid, "입장/퇴장을 하나 제작 하였습니다.");
				    else
				        SendMessage(playerid, "더 이상 건물을 생성할 자리가 없습니다.");
			...
			else
			{
		        if(listitem == 0)
		        {
		            if(PlayerInfo[playerid][pCreEnterID] == -1)
		            {
			            for(new e = 0; e < MAX_ENTER; e ++)
			            {
			                if(!EnterInfo[e][eCreate])
			                {
								...
								SendMessage(playerid, "이 장소는 건물의 입구로 지정 되었습니다.");
								SendMessage(playerid, "[/좌표이동]으로 건물 내부로 이동하여 출구 앞에서 [/건물관리]을 타이핑해주세요.");
								return 1;
			                }
			            }
					}
					else
					{
					    ...
					    ShowPlayerDialog(playerid, DIALOG_BUILD, DIALOG_STYLE_LIST, "건물생성", "건물\n목욕탕\n기본", "확인","재설정");
						return 1;
					}
		        }
		        if(listitem == 1)
		        {
		            if(PlayerInfo[playerid][pCreEnterID] == -1)
		            {
		                ...
		                ShowPlayerDialog(playerid, DIALOG_BUILD_EDIT, DIALOG_STYLE_LIST, "건물편집", string, "편집", "이전");
		                return 1;
		            }
					else
					{
					    ...
					    PlayerInfo[playerid][pCreEnterPos2][0] = 0;PlayerInfo[playerid][pCreEnterPos2][1] = 0;PlayerInfo[playerid][pCreEnterPos2][2] = 0;
			          	PlayerInfo[playerid][pCreEnterInt1] = 0; PlayerInfo[playerid][pCreEnterVW1] = 0; PlayerInfo[playerid][pCreEnterID] = -1;
			          	PlayerInfo[playerid][pCreEnterInt2] = 0; PlayerInfo[playerid][pCreEnterVW2] = 0; PlayerInfo[playerid][pCreEnterName] = 0;
			          	OnPlayerCommandText(playerid, "/건물관리");
			          	return 1;
					}
		        }
		        if(listitem == 2)
		        {
	                ...
	                ShowPlayerDialog(playerid, DIALOG_BUILD_DELETE, DIALOG_STYLE_LIST, "건물삭제", string, "삭제", "이전");
					return 1;
		        }
			}
	    }
	...
	if(dialogid == DIALOG_BUILD_DELETE)
	{
	    if(response)
	    {
	        ...
	        format(string, sizeof(string), "건물이 삭제 되었습니다. [ID : %d]",EnterInfo[EnterID][eID]);
	        SendMessage(playerid, string);
        ...
        if(response)
        {
            ...
            if(PlayerInfo[playerid][pEnterEdit] == 1)
		    {
		        ...
		        if(!strlen(inputtext))
		        {
			        ...
			        format(string, sizeof(string), ""#C_GRAY"\t- %s -\n\n"#C_WHITE"새로운 건물 이름을 지정해주세요.",EnterInfo[EnterID][eName]);
			        ShowPlayerDialog(playerid, DIALOG_BUILD_EDIT, DIALOG_STYLE_INPUT, "건물 이름변경", string, "확정", "이전");
			        return 1;
		        }
		        else
		        {
		            strmid(EnterInfo[EnterID][eName], inputtext, 0, 128, 128);
		            format(string, sizeof(string), "건물 이름이 %s로 바뀌었습니다. [ID : %d]",EnterInfo[EnterID][eName],EnterInfo[EnterID][eID]);
					SendMessage(playerid, string);
			...
			else if(PlayerInfo[playerid][pEnterEdit] == 2)
			{
	            ...
	            format(string, sizeof(string), "건물 종류가 %d타입으로 바뀌었습니다. [ID : %d]",EnterInfo[EnterID][eType], EnterInfo[EnterID][eID]);
				SendMessage(playerid, string);
			    SaveEnter(EnterID);
				return 1;
			}
			if(PlayerInfo[playerid][pEnterList] == -1)
			{
	            ...
	            PlayerInfo[playerid][pEnterList] = PlayerInfo[playerid][pEnterNumber][listitem - 1]; //-1하는 이유 listitem = 0 건물이 아니고 LIST이기 때문.
	            ShowPlayerDialog(playerid, DIALOG_BUILD_EDIT, DIALOG_STYLE_LIST, "건물편집", "건물 이름변경\n입장위치\n퇴장위치\n건물종류", "확인", "이전");
				return 1;
			}
			else
			{
			    ...
			    if(listitem == 1)
			    {
					...
					format(string, sizeof(string), "입장 좌표가 %f,%f,%f으로 바뀌었습니다. [ID : %d] [Interior : %d] [VirtualWorld : %d]",EnterInfo[EnterID][ePosX],EnterInfo[EnterID][ePosY],EnterInfo[EnterID][ePosZ],EnterInfo[EnterID][eID],EnterInfo[EnterID][eInterior],EnterInfo[EnterID][eVirtualWorld]);
					SendMessage(playerid, string);
					DestroyPickup(EnterInfo[EnterID][ePickup]);
					Delete3DTextLabel(EnterInfo[EnterID][e3DLabel]);
					EnterInfo[EnterID][ePickup] = CreatePickup(EnterInfo[EnterID][epickupid],1,EnterInfo[EnterID][ePosX],EnterInfo[EnterID][ePosY],EnterInfo[EnterID][ePosZ],EnterInfo[EnterID][eVirtualWorld]);
			    ...
			    if(listitem == 2)
			    {
			        ...
			        format(string, sizeof(string), "퇴장 좌표가 %f,%f,%f으로 바뀌었습니다. [ID : %d] [Interior : %d] [VirtualWorld : %d]",EnterInfo[EnterID][eInPosX],EnterInfo[EnterID][eInPosY],EnterInfo[EnterID][eInPosZ],EnterInfo[EnterID][eID],EnterInfo[EnterID][eInInterior],EnterInfo[EnterID][eInVirtualWorld]);
					SendMessage(playerid, string);
			    ...
			    if(listitem == 3)
		    	{
			        ...
			        ShowPlayerDialog(playerid, DIALOG_BUILD_EDIT, DIALOG_STYLE_LIST, "건물종류", "건물\n목욕탕\n기본", "확인", "이전");
			        return 1;
			    }
			}
        }
	    ...
	    if(response)
	    {
	        ...
	        if(PlayerInfo[playerid][pGangzoneCre] == 3)
	        {
	            if(strlen(inputtext))
	            {
					...
					if(CreateGangZone(PlayerInfo[playerid][pGangzoneS_PosX],PlayerInfo[playerid][pGangzoneS_PosY],PlayerInfo[playerid][pGangzoneE_PosX],PlayerInfo[playerid][pGangzoneE_PosY],inputtext) == 1)
					    format(string, sizeof(string), "갱존이 생성 되었습니다. [갱존명 : %s]",inputtext);
					else format(string, sizeof(string), "갱존 생성이 취소되었습니다. [이유 : 갱존 생성 자리 부족]");
				    SendMessage(playerid, string);
				    return 1;
	            }
	            else
	            {
				    ...
				    ShowPlayerDialog(playerid, DIALOG_GANGZONE, DIALOG_STYLE_INPUT, "갱존생성", "이 갱존의 이름을 정해주세요.","확정","취소");
	        ...
	        if(listitem == 0)
	        {
				if(PlayerInfo[playerid][pGangzoneCre] == 1)
				{
				    if(listitem == 0)
				    {
					    ...
					    SendMessage(playerid, "갱존을 끝낼 오른쪽 상단에서 '/갱존관리'를 타이핑 해주세요.");
				...
				else
				{
		            for(new g = 0; g < MAX_GANGZONE; g ++)
					{
					    ...
					    {
	               			SendMessage(playerid, "갱존을 시작할 갱존 왼쪽 하단에서 '/갱존관리'를 타이핑 해주세요.");
	               			PlayerInfo[playerid][pGangzoneCre] = 1;
	               			break;
		                }
		                else if(g == MAX_GANGZONE-1)
		                    return SendErrorMessage(playerid, "더 이상 갱존을 추가할 자리가 없습니다.");
		            }
				}
	        }
	        if(listitem == 1)
	        {
				...
				else
				{
	                ...
	                ShowPlayerDialog(playerid, DIALOG_GANGZONE_EDIT, DIALOG_STYLE_LIST, "갱존편집", string, "편집", "이전");
		            return 1;
				}
                return 1;
	        }
	        if(listitem == 2)
	        {
                ...
                ShowPlayerDialog(playerid, DIALOG_GANGZONE_DELETE, DIALOG_STYLE_LIST, "갱존삭제", string, "삭제", "이전");
	            return 1;
	        }
	    }
	    else
	...
	if(dialogid == DIALOG_GANGZONE_DELETE)
	{
		if(response)
		{
		    ...
		    format(string, sizeof(string), "갱존을 삭제 하였습니다. [이름 : %s] [번호 : %d]",GangzoneInfo[GangzoneID][gName],GangzoneInfo[GangzoneID][gNumber]);
		    SendMessage(playerid, string);
		    DeleteGangzone(GangzoneID);
		}
		else
		{
		    OnPlayerCommandText(playerid, "/갱존관리");
		}
		return 1;
	}
	    ...
	    if(response)
	    {
	        ...
	        if(PlayerInfo[playerid][pGangzoneNameEdit] == 1)
	        {
	            if(strlen(inputtext))
	            {
		            ...
		            format(string, sizeof(string), "갱존 이름이 %s로 바뀌었습니다. [ID : %d]",inputtext, GangzoneID);
		            SendMessage(playerid, string);
		            PlayerInfo[playerid][pGangzoneNameEdit] = 0;
		        ...
		        {
		            ShowPlayerDialog(playerid, DIALOG_GANGZONE_EDIT, DIALOG_STYLE_INPUT, "갱존 이름변경", "변경하실 갱존 이름을 기재해주세요.", "확인","이전");
	        ...
	        if(PlayerInfo[playerid][pGangzoneList] == -1)
			{
	        	...
	        	ShowPlayerDialog(playerid, DIALOG_GANGZONE_EDIT, DIALOG_STYLE_LIST, "갱존편집", "갱존 이름변경\n최소좌표 변경\n최대좌표 변경", "편집", "이전");
				return 1;
	        }
	        else
	        {
				...
				if(listitem == 1)
				{
				    ...
				    format(string, sizeof(string), "갱존의 최소 좌표를 %f,%f으로 변경하였습니다. [이름 : %s] [번호 : %d]",GangzoneInfo[GangzoneID][gS_PosX], GangzoneInfo[GangzoneID][gS_PosY],GangzoneInfo[GangzoneID][gName],GangzoneInfo[GangzoneID][gNumber]);
					SendMessage(playerid, string);
				...
				if(listitem == 2)
				{
				    ...
				    format(string, sizeof(string), "갱존의 최대 좌표를 %f,%f으로 변경하였습니다. [이름 : %s] [번호 : %d]",GangzoneInfo[GangzoneID][gE_PosX], GangzoneInfo[GangzoneID][gE_PosY],GangzoneInfo[GangzoneID][gName],GangzoneInfo[GangzoneID][gNumber]);
                    SendMessage(playerid, string);
					ResetGangzone(GangzoneID);
                    SaveGangzone(GangzoneInfo[GangzoneID][gNumber]);
					return 1;
	...
	if(dialogid == DIALOG_OCCUPY_AREA)
	{
	    if(response)
	    {
				...
				if(CheckGangzone(playerid) == 1)
				{
		            for(new o = 0; o < MAX_OCCUPY_AREA; o ++)
		            {
		                if(!Occupy_AreaInfo[o][oCreate])
		                {
		            		...
		            		ShowPlayerDialog(playerid, DIALOG_OCCUPY_AREA_CRE, DIALOG_STYLE_INPUT, "점령지 생성",""#C_GRAY"\t\t새로운 점령지 생성\n\n"#C_WHITE"새로 추가할 점령지 이름을 기재한 후 '생성'을 클릭해주세요.","생성","취소");
							return 1;
		                }
		            }
				}
			...
			if(listitem == 1)
			{
				...
				ShowPlayerDialog(playerid, DIALOG_OCCUPY_AREA_EDIT, DIALOG_STYLE_LIST, "점령지 편집", string, "편집", "이전");
			}
			if(listitem == 2)
			{
			    new Number = 0, string[512]; PlayerInfo[playerid][pOccupyAreaList] = -1; PlayerInfo[playerid][pOccupyAreaEditType] = 0;
				...
				ShowPlayerDialog(playerid, DIALOG_OCCUPY_AREA_DELETE, DIALOG_STYLE_LIST, "점령지 삭제", string, "삭제", "이전");
			}
	    }
	    else
	    {
	...
	if(dialogid == DIALOG_OCCUPY_AREA_DELETE)
	{
	    if(response)
	    {
	        ...
	        else
	        {
		        ...
		        format(string, sizeof(string), "건물이 삭제 되었습니다. [이름 : %s] [번호 : %d]",Occupy_AreaInfo[PlayerInfo[playerid][pOccupyAreaList]][oName],PlayerInfo[playerid][pOccupyAreaList]);
		        SendMessage(playerid, string);
	    ...
	    {
	        OnPlayerCommandText(playerid, "/점령지관리");
	    }
	    return 1;
	}
	if(dialogid == DIALOG_OCCUPY_AREA_EDIT)
	{
	    if(response)
	    {
	        ...
	        if(PlayerInfo[playerid][pOccupyAreaEditType] == 1)
	        {
				if(strlen(inputtext))
				{
				    ...
				    format(string, sizeof(string), "점령지 이름이 %s로 바뀌었습니다. [ID : %d]",Occupy_AreaInfo[OccupyAreaID][oName], Occupy_AreaInfo[OccupyAreaID][oNumber]);
					SendMessage(playerid, string);
				...
				else
				{
				    ...
				    ShowPlayerDialog(playerid, DIALOG_OCCUPY_AREA_EDIT, DIALOG_STYLE_LIST, "점령지 편집", ""#C_GRAY"\t\t점령지 이름변경\n\n"#C_WHITE"변경하실 점령지 이름을 기재하신 후, '확정'을 눌러주세요.", "확정", "이전");
				}
	            return 1;
	        }
	        else if(PlayerInfo[playerid][pOccupyAreaEditType] == 2)
	        {
	            if(listitem == 0)
	            {
	                ...
	                if(g == -1) return SendErrorMessage(playerid, "주변에 갱존이 없습니다.");
	                SetAreaColorEx(g, 1);
	                new string[128];
	                format(string, sizeof(string), "어드민 %s님이 %s점령지의 소속국가를 미국으로 변경 하였습니다.",PlayerInfo[playerid][pAdminName],Occupy_AreaInfo[OccupyAreaID][oName]);
	                SendAdminMessage(string);
	                //SaveOccupyArea(OccupyAreaID);
	            }
	            if(listitem == 1)
	            {
	                ...
	                SetAreaColorEx(g, 2);
	                new string[128];
	                format(string, sizeof(string), "어드민 %s님이 %s점령지의 소속국가를 러시아로 변경 하였습니다.",PlayerInfo[playerid][pAdminName],Occupy_AreaInfo[OccupyAreaID][oName]);
	                SendAdminMessage(string);
	                //SaveOccupyArea(OccupyAreaID);
	            }
	        }
	        else
	        {
		        if(PlayerInfo[playerid][pOccupyAreaList] == -1)
		        {
			        ...
			        else
			        {
			            ...
			            ShowPlayerDialog(playerid, DIALOG_OCCUPY_AREA_EDIT, DIALOG_STYLE_LIST, "점령지 편집", "점령지 이름변경\n점령지 위치변경\n점령지 국가변경", "확인", "이전");
			        }
				}
				else
				{
				    ...
				    if(listitem == 1)
				    {
				        if(CheckGangzone(playerid) == 1)
						{
					        ...
					        format(string, sizeof(string), "점령지 위치가 %f,%f,%f로 바뀌었습니다. [ID : %d]",Occupy_AreaInfo[OccupyAreaID][oPosX], Occupy_AreaInfo[OccupyAreaID][oPosY], Occupy_AreaInfo[OccupyAreaID][oPosZ],Occupy_AreaInfo[OccupyAreaID][oPosX], Occupy_AreaInfo[OccupyAreaID][oPosY], Occupy_AreaInfo[OccupyAreaID][oNumber]);
							SendMessage(playerid, string);
							DestroyPickup(Occupy_AreaInfo[OccupyAreaID][oPickup]);
			    			Occupy_AreaInfo[OccupyAreaID][oPickup] = CreatePickup(2914,1,Occupy_AreaInfo[OccupyAreaID][oPosX],Occupy_AreaInfo[OccupyAreaID][oPosY],Occupy_AreaInfo[OccupyAreaID][oPosZ] - PickupDown,0);
							SaveOccupyArea(OccupyAreaID);
				    ...
				    if(listitem == 2)
					{
				        ...
				        ShowPlayerDialog(playerid, DIALOG_OCCUPY_AREA_EDIT, DIALOG_STYLE_LIST, "점령지 편집", "미국\n러시아", "확인", "이전");
				    }
				}
			}
			return 1;
	    ...
	    {
	        if(PlayerInfo[playerid][pOccupyAreaEditType] == 1)
	            return ShowPlayerDialog(playerid, DIALOG_OCCUPY_AREA_EDIT, DIALOG_STYLE_LIST, "점령지 편집", "점령지 이름변경\n점령지 위치변경", "확인", "이전");
	        if(PlayerInfo[playerid][pOccupyAreaList] == -1)
	            return OnPlayerCommandText(playerid, "/점령지관리");
	        else
			{
	            new Number = 0, string[512]; PlayerInfo[playerid][pOccupyAreaList] = -1; PlayerInfo[playerid][pOccupyAreaEditType] = 0;
				strmid(string, ""#C_GRAY"ID\tName"#C_WHITE"", 0, 512, 512);
	            ...
	            if(CheckGangzone(playerid) == 1)
				{
		            if(strlen(inputtext))
		            {
			            ...
			            if(CreateOccupyArea(PlayerInfo[playerid][pOccupyAreaPosX], PlayerInfo[playerid][pOccupyAreaPosY], PlayerInfo[playerid][pOccupyAreaPosZ],inputtext) == 1)
			                format(string, sizeof(string), "점령지를 하나 추가하였습니다. %f,%f,%f [이름 : %s]",PlayerInfo[playerid][pOccupyAreaPosX], PlayerInfo[playerid][pOccupyAreaPosY], PlayerInfo[playerid][pOccupyAreaPosZ],inputtext);
			            else
			                format(string, sizeof(string), "더 이상 점령지를 추가 할 수 없습니다.");
			            SendMessage(playerid, string);
					}
					else
					{
	        ...
	        if(listitem == 0)
	        {
				for(new p_t = 1; p_t < MAX_PLATOON; p_t ++)
				{
					if(!PlatoonInfo[p_t][p_tCreate])
					{
					    ...
					    ShowPlayerDialog(playerid, DIALOG_PLATOON_CRE, DIALOG_STYLE_INPUT, "소대추가", ""#C_GRAY"\t\t\t- 소대추가 -\n"#C_WHITE"이 소대의 소대번호를 적어주신 후 '확인'을 클릭해주세요", "확인","이전");
						return 1;
					}
					else if(p_t == MAX_PLATOON-1)
						return SendErrorMessage(playerid, "소대를 더이상 만들 수 없습니다.");
				}
	        }
	        if(listitem == 1)
	        {
				...
				for(new p_t = 1; p_t < MAX_PLATOON; p_t ++)
				{
					...
					if(PlatoonInfo[p_t][p_tCountry] == 1) strmid(country, "미국", 0, 52, 52);
					else if(PlatoonInfo[p_t][p_tCountry] == 2) strmid(country, "러시아", 0, 52, 52);
				...
				ShowPlayerDialog(playerid, DIALOG_PLATOON_EDIT, DIALOG_STYLE_LIST, "소대편집", string, "편집", "이전");
	        }
	        if(listitem == 2)
	        {
	            new Number = 1, string[512], country[52]; PlayerInfo[playerid][pPlatoonEditState] = -1; PlayerInfo[playerid][pPlatoonEditList] = -1;
				...
				ShowPlayerDialog(playerid, DIALOG_PLATOON_DELETE, DIALOG_STYLE_LIST, "소대삭제", string, "삭제", "이전");
	        }
	    }
	    else
	    {
	...
	if(dialogid == DIALOG_PLATOON_DELETE)
	{
		if(response)
		{
		    ...
		    else
		    {
		        ...
		        format(string, sizeof(string), "어드민 %s님이 소대 하나를 삭제 하였습니다. [ID : %d] [Name : %s]",PlayerInfo[playerid][pAdminName],PlatoonID,PlatoonInfo[PlatoonID][p_tName]);
		        SendAdminMessage(string);
	...
	if(dialogid == DIALOG_PLATOON_EDIT)
	{
		if(response)
		{
		    ...
		    if(PlayerInfo[playerid][pPlatoonEditList] == -1)
			{
			    ...
			    {
					PlayerInfo[playerid][pPlatoonEditList] = PlayerInfo[playerid][pPlatoonEditNumber][listitem];//팩션 넘버는 1부터 시작하기 때문에 Listitem - 1 을 해 줄 필요가 없음
					ShowPlayerDialog(playerid, DIALOG_PLATOON_EDIT, DIALOG_STYLE_LIST, "소대편집", "소대 이름변경\n소대 번호변경\n소대 역할변경\n소대 국가변경", "확인","이전");
					return 1;
				}
			}
		    else if(PlayerInfo[playerid][pPlatoonEditList])
		    {
			    if(PlayerInfo[playerid][pPlatoonEditState] == 1)
			    {
			        ...
			        format(string, sizeof(string), "어드민 %s님에 의해 소대 이름이 변경되었습니다. [ID : %d] [Name : %s]",PlayerInfo[playerid][pAdminName],PlatoonID,PlatoonInfo[PlatoonID][p_tName]);
			        SendAdminMessage(string);
				        ...
				        for(new p_t = 1; p_t < MAX_PLATOON; p_t ++)
				            if(PlatoonInfo[p_t][p_tCountry] == PlatoonInfo[PlatoonID][p_tCountry])
								if(PlatoonInfo[p_t][p_tType] == _FactionName[listitem][_fType]){
								    SendErrorMessage(playerid, "일반소대 외 부대역할은 다른 부대와 중복되면 안 됩니다.");
								    new string[128];
							        ...
							        ShowPlayerDialog(playerid, DIALOG_PLATOON_EDIT, DIALOG_STYLE_LIST, "소대편집", string, "확인","이전"); return 0;}
				    new string[256];
			        PlatoonInfo[PlatoonID][p_tType] = _FactionName[listitem][_fType];
			        format(string, sizeof(string), "어드민 %s님에 의해 소대 역할이 %s로 바뀌었습니다. [Name : %s]",PlayerInfo[playerid][pAdminName],_FactionName[listitem][_fName], PlatoonInfo[PlatoonID][p_tName]);
			        SendAdminMessage(string);
			        SavePlatoon(PlatoonID);
			        return 1;
			    }
			    else if(PlayerInfo[playerid][pPlatoonEditState] == 3)
			    {
					...
					for(new p_t = 1; p_t < MAX_PLATOON; p_t ++)
						if(PlatoonInfo[p_t][p_t_Number] == number){
							SendErrorMessage(playerid, "이미 그 소대번호로 등록된 소대가 존재합니다.");
							ShowPlayerDialog(playerid, DIALOG_PLATOON_EDIT, DIALOG_STYLE_INPUT, "소대편집", ""#C_GRAY"\t\t- 소대편집 -\n"#C_WHITE"이 소대의 소대번호를 적어주신 후 '확인'을 클릭해주세요", "확인","이전"); return 0;}
					if(0 < number < 1000)
					{
				        new PlatoonID = PlayerInfo[playerid][pPlatoonEditList], string[256];
				        PlatoonInfo[PlatoonID][p_t_Number] = number;
				        format(string, sizeof(string), "어드민 %s님에 의해 소대 번호가 %d로 변경되었습니다. [Name : %s]",PlayerInfo[playerid][pAdminName],number, PlatoonInfo[PlatoonID][p_tName]);
				        SendAdminMessage(string);
				        SavePlatoon(PlatoonID);
					}
					else
					{
					    ShowPlayerDialog(playerid, DIALOG_PLATOON_EDIT, DIALOG_STYLE_INPUT, "소대편집", ""#C_GRAY"\t\t- 소대편집 -\n"#C_WHITE"이 소대의 소대번호를 적어주신 후 '확인'을 클릭해주세요", "확인","이전");
					    SendErrorMessage(playerid, "소대 번호는 1 ~ 999까지만 가능합니다.");
					}
					return 1;
			    }
			    else if(PlayerInfo[playerid][pPlatoonEditState] == 4)
			    {
			        ...
			        new country[52];
			        if(PlatoonInfo[PlatoonID][p_tCountry] == 1) strmid(country, "미국", 0, 52, 52);
					else if(PlatoonInfo[PlatoonID][p_tCountry] == 2) strmid(country, "러시아", 0, 52, 52);
                    format(string, sizeof(string), "어드민 %s님에 의해 소대국가가 %s(으)로 변경하였습니다. [Name : %s]",PlayerInfo[playerid][pAdminName],country, PlatoonInfo[PlatoonID][p_tName]);
			        SendAdminMessage(string);
			        SavePlatoon(PlatoonID);
			    }
			    else if(PlayerInfo[playerid][pPlatoonEditState] == -1)
			    {
			        if(listitem == 0)
				    {
				        ...
				        ShowPlayerDialog(playerid, DIALOG_PLATOON_EDIT, DIALOG_STYLE_INPUT, "소대편집", ""#C_GRAY"\t\t\t- 소대 이름편집 -\n\n"#C_WHITE"이 소대의 새로운 이름을 정하신 뒤 '확인'을 클릭해주세요.", "확인","이전");
				    }
				    if(listitem == 1)
				    {
				    ...
				    if(listitem == 2)
				    {
				        ...
				        ShowPlayerDialog(playerid, DIALOG_PLATOON_EDIT, DIALOG_STYLE_LIST, "소대편집", string, "확인","이전");
				    }
				    if(listitem == 3)
				    {
				        PlayerInfo[playerid][pPlatoonEditState] = 4;
					    ShowPlayerDialog(playerid, DIALOG_PLATOON_EDIT, DIALOG_STYLE_LIST, "소대편집", "미국\n러시아", "확인","이전");
				    }
			    }
			}
		}
		...
		{
		    if(PlayerInfo[playerid][pPlatoonEditList] == -1)
				OnPlayerCommandText(playerid, "/소대관리");
			else
			{
                if(PlayerInfo[playerid][pPlatoonEditState] == -1)
                {
                ...
                else{ PlayerInfo[playerid][pPlatoonEditState] = -1; ShowPlayerDialog(playerid, DIALOG_PLATOON_EDIT, DIALOG_STYLE_LIST, "소대편집", "소대 이름변경\n소대 번호변경\n소대 역할변경", "확인","이전");}
			}
		}
		return 1;
	}
	if(dialogid == DIALOG_PLATOON_CRE)
	{
	    if(response)
	    {
	        ...
	        if(PlayerInfo[playerid][pPlatoonCre] == 1)
	        {
				...
				if(0 < number < 1000)
				{
				    ...
				    ShowPlayerDialog(playerid, DIALOG_PLATOON_CRE, DIALOG_STYLE_INPUT, "소대추가", ""#C_GRAY"\t\t\t- 소대추가 -\n"#C_WHITE"추가하실 소대의 소대이름을 적은 후 '확인'을 클릭해주세요.", "확인","이전");
				}
				else
				{
				    ShowPlayerDialog(playerid, DIALOG_PLATOON_CRE, DIALOG_STYLE_INPUT, "소대추가", ""#C_GRAY"\t\t\t- 소대추가 -\n"#C_WHITE"이 소대의 소대번호를 적어주신 후 '확인'을 클릭해주세요", "확인","이전");
	        ...
	        else if(PlayerInfo[playerid][pPlatoonCre] == 2)
	        {
				if(strlen(inputtext))
				{
				    ...
				    ShowPlayerDialog(playerid, DIALOG_PLATOON_CRE, DIALOG_STYLE_LIST, "소대추가", "미국\n러시아", "확인","이전");
				}
				else
				{
				    PlayerInfo[playerid][pPlatoonCre] = 2;
			...
			else if(PlayerInfo[playerid][pPlatoonCre] == 3)
	        {
                ...
                ShowPlayerDialog(playerid, DIALOG_PLATOON_CRE, DIALOG_STYLE_LIST, "소대추가", string, "확인","이전");
	        }
			else if(PlayerInfo[playerid][pPlatoonCre] == 4)
			{
			    for(new p_t = 1; p_t < MAX_PLATOON; p_t ++)
			    {
					...
					{
						return SendErrorMessage(playerid, "소대번호가 다른 소대와 겹쳐 소대 생성이 취소되었습니다, 다시 시도해주세요.");
				    }
				}
				new string[256];
                ...
                {
   					format(string, sizeof(string), "어드민 %s님이 소대 하나를 만들었습니다. [Name : %s] [Type : %s]",PlayerInfo[playerid][pAdminName],PlayerInfo[playerid][pPlatoonCreName], _FactionName[listitem][_fName]);
					SendAdminMessage(string);
				}
                else
                {
                    format(string, sizeof(string), "더 이상 소대를 추가할 수 없습니다.");
					SendErrorMessage(playerid, string);
				}
			    return 1;
			}
	    ...
	    else
	    {
	        if(PlayerInfo[playerid][pPlatoonCre] == 1)
	            ...
	            PlayerInfo[playerid][pPlatoonCre] = 1; ShowPlayerDialog(playerid, DIALOG_PLATOON_CRE, DIALOG_STYLE_INPUT, "소대추가", ""#C_GRAY"\t\t\t- 소대추가 -\n"#C_WHITE"이 소대의 소대번호를 적어주신 후 '확인'을 클릭해주세요", "확인","이전");}
	        else if(PlayerInfo[playerid][pPlatoonCre] == 3){
	            PlayerInfo[playerid][pPlatoonCre] = 2; ShowPlayerDialog(playerid, DIALOG_PLATOON_CRE, DIALOG_STYLE_INPUT, "소대추가", ""#C_GRAY"\t\t\t- 소대추가 -\n"#C_WHITE"추가하실 소대의 소대이름을 적은 후 '확인'을 클릭해주세요.", "확인","이전");}
	    }
	    return 1;
	}
	if(dialogid == DIALOG_TAKE_WEAPON)
	{
	    if(response)
	    {
	        if(listitem == 0)
	        {
			    ...
			    format(string, sizeof(string), "/me %s를 줍는다. [%d개]", string);
			    OnPlayerCommandText(playerid, string);
			    return 1;
	        }
	        if(listitem == 1)
	    ...
	    if(response)
	    {
	        ...
	        if(IsPlayerLogin(giveplayerid))
	        {
	            if(strcmp(PlayerInfo[playerid][pgClickName], PlayerNameEx(giveplayerid), true) == 0)
	            {
			        ...
			        if(listitem == 2)
			        {
			            ShowPlayerDialog(playerid, DIALOG_INFOEDIT, DIALOG_STYLE_LIST, "정보수정", "성별변경\n국가변경\n계급설정\n소대설정", "확인", "이전");
			        }
			        return 1;
				}
				else
				{
				    SendErrorMessage(playerid, "선택된 플레이어번호와 이름이 맞지 않습니다. 다시 시도해주세요.");
				}
			}
			else
			{
			    SendErrorMessage(playerid, "선택된 플레이어는 현재 로그인을 하지 않았습니다. 다시 시도해주세요.");
			}
	    }
	    else
	    {
	...
	if(dialogid == DIALOG_INFOEDIT)
	{
	            ...
	            if(strcmp(PlayerInfo[playerid][pgClickName], PlayerNameEx(giveplayerid), true) == 0)
	            {
	                ...
	                if(listitem == 0)
	                {
	                    if(PlayerInfo[giveplayerid][pSex] == 1)
	                    {
	                        ...
	                        format(string, sizeof(string), "어드민 %s님이 %s님의 성별을 여성으로 변경 하였습니다.", PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
	                        SendAdminMessage(string);
	                        format(string, sizeof(string), "어드민 %s님에 의해 성별이 여성으로 변경 되었습니다.", PlayerInfo[playerid][pAdminName]);
	                        SendMessage(giveplayerid, string);
	                    }
	                    else if(PlayerInfo[giveplayerid][pSex] == 2)
	                    {
	                        PlayerInfo[giveplayerid][pSex] = 1;
	                        format(string, sizeof(string), "어드민 %s님이 %s님의 성별을 남성으로 변경 하였습니다.", PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
	                        SendAdminMessage(string);
	                        format(string, sizeof(string), "어드민 %s님에 의해 성별이 남성으로 변경 되었습니다.", PlayerInfo[playerid][pAdminName]);
	                        SendMessage(giveplayerid, string);
	                    }
	                    OnPlayerSpawn(giveplayerid);
	                }
	                if(listitem == 1)
	                {
	                    if(PlayerInfo[giveplayerid][pCountry] == 1)
	                    {
	                        ...
	                        format(string, sizeof(string), "어드민 %s님이 %s님의 국가를 러시아로 변경 하였습니다.", PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
	                        SendAdminMessage(string);
	                        format(string, sizeof(string), "어드민 %s님에 의해 당신의 국가가 러시아로 변경되었습니다.", PlayerInfo[playerid][pAdminName]);
	                        SendMessage(giveplayerid, string);
	                    }
	                    else if(PlayerInfo[giveplayerid][pCountry] == 2)
	                    {
	                        ChangePlayerCountry(giveplayerid, 1);
	                        format(string, sizeof(string), "어드민 %s님이 %s님의 국가를 미국으로 변경 하였습니다.", PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid));
	                        SendAdminMessage(string);
	                        format(string, sizeof(string), "어드민 %s님에 의해 당신의 국가가 미국으로 변경되었습니다.", PlayerInfo[playerid][pAdminName]);
	                        SendMessage(giveplayerid, string);
	                    }
	                    OnPlayerSpawn(giveplayerid);
	                }
	                if(listitem == 2)
	                {
	                    ...
	                    ShowPlayerDialog(playerid, DIALOG_INFOEDIT_CLASS, DIALOG_STYLE_LIST, "계급설정", string, "확인", "이전");
	                }
	                if(listitem == 3)
	                {
	                }
		...
		{
		    ShowPlayerDialog(playerid, DIALOG_PLAYERCLICK, DIALOG_STYLE_LIST, PlayerNameEx(PlayerInfo[playerid][pgClickID]), "출두\n소환\n정보수정", "확인", "취소");
		}
		return 1;
	}
	    ...
	    if(response)
	    {
	        ...
	        if(IsPlayerLogin(giveplayerid))
	        {
	            if(strcmp(PlayerInfo[playerid][pgClickName], PlayerNameEx(giveplayerid), true) == 0)
	            {
                    ...
                    format(string, sizeof(string), "어드민 %s님이 %s님의 계급을 %s로 변경 하였습니다.", PlayerInfo[playerid][pAdminName], PlayerNameEx(giveplayerid), ArmyClass(listitem));
                    SendAdminMessage(string);
                    format(string, sizeof(string), "어드민 %s님에 의해 계급이 %s로 변경되었습니다.", PlayerInfo[playerid][pAdminName], ArmyClass(listitem));
                    SendMessage(giveplayerid, string);
	            }
	            else
				{
	...
	if(dialogid == DIALOG_INVENTORY)
	{
	    if(response)
	    {
	        ...
	        else
	        {
	            ...
	            format(string, sizeof(string), ""#C_GRAY"[상품명 %s 개수 %d]"#C_WHITE"\n아이템사용\n아이템버리기",ItemInfo[PlayerInfo[playerid][pI_ItemID][PlayerInfo[playerid][pI_Select]]][_ItemName],
				PlayerInfo[playerid][pI_Number][PlayerInfo[playerid][pI_Select]]);
	            ShowPlayerDialog(playerid, DIALOG_INVENTORY_USE, DIALOG_STYLE_LIST, "가방", string, "확인", "이전");
	        }
	    }
	    else
	    {
	...
	if(dialogid == DIALOG_INVENTORY_USE)
	{
	    if(response)
	    {
	        ...
	        if(listitem == 0)
	        {
	            new string[256]; format(string, sizeof(string), ""#C_GRAY"[상품명 %s 개수 %d]"#C_WHITE"\n아이템사용\n아이템버리기",ItemInfo[PlayerInfo[playerid][pI_ItemID][PlayerInfo[playerid][pI_Select]]][_ItemName],
				PlayerInfo[playerid][pI_Number][PlayerInfo[playerid][pI_Select]]);
	            ShowPlayerDialog(playerid, DIALOG_INVENTORY_USE, DIALOG_STYLE_LIST, "가방", string, "확인", "이전");
	        }
			if(listitem == 1)
			    ...
			    if(PlayerInfo[playerid][pI_Number][select] > 0)
			    {
				    switch(PlayerInfo[playerid][pI_ItemID][select])
				    {
						case 47 :
						{
							...
							strmid(string, ""#C_GRAY"플레이어 번호\t이름"#C_WHITE"", 0, 512, 512);
							format(str, sizeof(str), "모르핀 %d개",PlayerInfo[playerid][pI_Number][select]);
							for(new i = 0; i < MAX_PLAYERS; i ++)
							{
							...
							ShowPlayerDialog(playerid, DIALOG_MORPHINE, DIALOG_STYLE_LIST, str, string, "확인", "이전");
						}
						case 48 : //알약
						{
						    if(PlayerInfo[playerid][pPillEat] == 1) { new string[128]; format(string, sizeof(string), "다시 시도해주세요. [%d초]",PlayerInfo[playerid][pPillTimer]); SendErrorMessage(playerid, string); return 0; }
						    new Float:health;
						    GetPlayerHealth(playerid, health);
						    if(health >= 100) return SendErrorMessage(playerid, "더 이상 체력을 회복할 수 없습니다.");
					    ...
					    case 49 : //무전기
					    {
					        ...
					        new string[128];
					        format(string, sizeof(string), "현재 주파수 : %d\n"#C_RED" > "#C_WHITE"대화하기\n"#C_RED"> "#C_WHITE"주파수 설정", PlayerInfo[playerid][pPhoneFrequency]);
					        ShowPlayerDialog(playerid, DIALOG_PHONE_TALK, DIALOG_STYLE_LIST, "무전기", string, "확인", "취소");
					    }
					    case 50 : //C4
					    {
					        OnPlayerCommandText(playerid, "/폭탄설치");
					    }
					    case 51 : //예비탄약
					    {
					    ...
					    case 52 : //담배
					    {
							OnPlayerCommandText(playerid, "/담배");
					    }
					    case 53 : //맥주
					    {
                            OnPlayerCommandText(playerid, "/맥주");
					    }
				    }
				}
				else
			...
			if(listitem == 2)
			{
			    new string[128]; format(string, sizeof(string), "%s 버리기 [개수 %d개]",ItemInfo[PlayerInfo[playerid][pI_ItemID][PlayerInfo[playerid][pI_Select]]][_ItemName],
				PlayerInfo[playerid][pI_Number][PlayerInfo[playerid][pI_Select]]);
			    ShowPlayerDialog(playerid, DIALOG_INVENTORY_DELETE, DIALOG_STYLE_INPUT, string, ""#C_GRAY"\t\t - 아이템 버리기 - \n\n"#C_WHITE"버리실 개수를 적어주신 후 '확인'을 클릭해주세요.", "확인", "이전");
			}
		}
		else
		{
	...
	if(dialogid == DIALOG_INVENTORY_DELETE)
	{
		if(response)
		{
		    ...
		    else
		    {
				...
				PlayerInfo[playerid][pI_Number][PlayerInfo[playerid][pI_Select]]);
			    ShowPlayerDialog(playerid, DIALOG_INVENTORY_DELETE, DIALOG_STYLE_INPUT, string, ""#C_GRAY"\t\t - 아이템 버리기 - \n\n"#C_WHITE"버리실 개수를 적어주신 후 '확인'을 클릭해주세요.", "확인", "이전");
				format(string, sizeof(string), "1개 이상 %d개 이하 버릴 수 있습니다.",PlayerInfo[playerid][pI_Number][PlayerInfo[playerid][pI_Select]]);
				SendErrorMessage(playerid, string);
			}
		}
		else
	    ...
	    if(response)
	    {
	        ...
	        */
	        ...
	        else
	        {
	            ...
	            format(string, sizeof(string), "/모르핀 %d",giveplayerid);
	            OnPlayerCommandText(playerid, string);
	        }
	    }
	    else
	    ...
	    if(response)
	    {
	        if(PlayerInfo[playerid][pPhoneTakeMode] == 0)
	        {
				...
				if(listitem == 1)
				{
				    ...
				    ShowPlayerDialog(playerid, DIALOG_PHONE_TALK, DIALOG_STYLE_LIST, "무전기", "", "확인", "이전");
				}
				if(listitem == 2)
				{
			...
			else if(PlayerInfo[playerid][pPhoneTakeMode] == 2)
			{
			    ...
			    new string[128];
			    PlayerInfo[playerid][pPhoneTakeMode] = 0;
			    format(string, sizeof(string), "/주파수 %d",num);
			    OnPlayerCommandText(playerid, string);
			}
	    }
	    else
	    ...
	    if(response)
	    {
			...
			if(listitem == 0)
			{
			    if(PlayerInfo[playerid][pWork] == 0)
			    {
					...
					SendMessage(playerid, "무장을 하였습니다.");
				}
				else if(PlayerInfo[playerid][pWork] == 1)
				{
				    ...
				    SetPlayerArmourEx(playerid, 0.0);
				    PlayerInfo[playerid][pWork] = 0;
				    SendMessage(playerid, "무장을 해제하였습니다.");
				    ResetPlayerWeaponsEx(playerid);
				}
			}
			if(listitem == 1)
			{
			    ShowPlayerDialog(playerid, DIALOG_ARME_CLOTHES, DIALOG_STYLE_LIST, "환복", "군복\n사복", "확인", "이전");
			}
	    }
	    else
	    {
	...
	if(dialogid == DIALOG_PRODUCT)
	{
	    if(response)
	    {
	        ...
	        if(PlayerInfo[playerid][pProductEdit] == 0)
	        {
		        if(listitem == 0)
		        {
		            for(new i = 0; i < MAX_PRODUCT; i ++)
					{
					    if(!ProductInfo[i][p_dCreate])
							break;
						else if(i == MAX_PRODUCT -1) return SendErrorMessage(playerid, "더 이상 상품을 추가할 수 없습니다.");
				    }
		            ShowPlayerDialog(playerid, DIALOG_PRODUCT, DIALOG_STYLE_LIST, "상품생성", ""#C_RED"> "#C_WHITE"모르핀\n"#C_RED"> "#C_WHITE"무전기\n"#C_RED"> "#C_WHITE"C4\n"#C_RED"> "#C_WHITE"알약\n"#C_RED"> "#C_WHITE"예비탄약\n"#C_RED"> "#C_WHITE"담배\n"#C_RED"> "#C_WHITE"맥주", "확인", "이전");
		            PlayerInfo[playerid][pProductEdit] = 1;
		        }
		        if(listitem == 1)
		        {
					new string[512], number = 0;
					strmid(string, "번호\t상품번호\t가격\t상품이름", 0, 512, 512);
		            for(new i = 0; i < MAX_PRODUCT; i ++)
		            {
		            ...
		            ShowPlayerDialog(playerid, DIALOG_PRODUCT, DIALOG_STYLE_LIST, "상품편집", string, "확인", "이전");
		        ...
		        if(listitem == 2)
		        {
		            ...
		            ShowPlayerDialog(playerid, DIALOG_PRODUCT, DIALOG_STYLE_LIST, "상품삭제", string, "확인", "이전");
		        }
			}
			else if(PlayerInfo[playerid][pProductEdit] == 1)//생성
			{
			    new objectid;
			    ...
			    ShowPlayerDialog(playerid, DIALOG_PRODUCT, DIALOG_STYLE_INPUT, "상품생성", ""#C_WHITE"이 상품의 가격을 설정해주세요.", "확인", "이전");
			}
			else if(PlayerInfo[playerid][pProductEdit] == 2)//편집
			{
			    if(listitem == 0)
			    {
			        new string[512], number = 0;
			    ...
			    else
			    {
			    	...
			    	ShowPlayerDialog(playerid, DIALOG_PRODUCT, DIALOG_STYLE_LIST, "상품편집", "상품변경\n가격변경\n위치변경", "확인", "이전");
			    }
			}
			else if(PlayerInfo[playerid][pProductEdit] == 3)//상품삭제
			{
			    if(listitem == 0)
			    {
			        new string[512], number = 0;
			    ...
			    else
			    {
			        ...
			        new string[128];
			    	format(string, sizeof(string), "상품이 삭제 되었습니다. [이름 : %s] [번호 : %d]", ObjectNameEx(ProductInfo[id][p_dObjectID]),ProductInfo[id][p_dNumber]);
			    	SendMessage(playerid, string);
			    	
           			DeleteProduct(ProductInfo[id][p_dNumber]);
			    }
			}
			else if(PlayerInfo[playerid][pProductEdit] == 4)//생성확정
			{
				...
				if(0 < money <= 500)
				{
				    ...
				    for(new i = 0, l = sizeof(ItemInfo); i < l; i ++)
				    {
				    	...
				    	else if(i == l -1) return SendErrorMessage(playerid, "상품 생성 오류가 발생 하였습니다. 오류코드 : 1000");
					}
					format(string, sizeof(string), "상품 하나를 생성 하였습니다. [이름 : %d] [가격 : %d$]",name,money);

					if(CreateProduct(PlayerInfo[playerid][pProductOjbectID], money, pos[0], pos[1], pos[2]) == 1) SendMessage(playerid, string);
					else return SendErrorMessage(playerid, "더 이상 상품을 추가할 수 없습니다.");
				}
				else return SendErrorMessage(playerid, "가격은 1~500$까지 설정 가능합니다.");
			}
			else if(PlayerInfo[playerid][pProductEdit] == 5)
			{
			    if(listitem == 0)
			    {
			        ...
			        ShowPlayerDialog(playerid, DIALOG_PRODUCT, DIALOG_STYLE_LIST, "상품변경", ""#C_RED"> "#C_WHITE"모르핀\n"#C_RED"> "#C_WHITE"무전기\n"#C_RED"> "#C_WHITE"C4\n"#C_RED"> "#C_WHITE"알약\n"#C_RED"> "#C_WHITE"예비탄약\n"#C_RED"> "#C_WHITE"담배\n"#C_RED"> "#C_WHITE"맥주", "확인", "이전");
			    }
			    if(listitem == 1)
			    {
			        PlayerInfo[playerid][pProductEdit] = 7;
			        new string[128];
			        format(string, sizeof(string), "\t"#C_GRAY"- %s 가격변경 -\n"#C_WHITE"원하시는 상품의 가격을 설정해주세요.");
			        ShowPlayerDialog(playerid, DIALOG_PRODUCT, DIALOG_STYLE_INPUT, "가격변경", string, "확인", "이전");
			    }
			    if(listitem == 2)
			    {
			    	...
			    	new string[128];
			    	format(string, sizeof(string), "상품의 위치가 변경 되었습니다. [이름 : %s] [%.2f,%.2f,%.2f]",
			...
			else if(PlayerInfo[playerid][pProductEdit] == 6)//아이템변경
			{
			    ...
			    format(string, sizeof(string), "상품이 %s로 변경 되었습니다. [번호 : %d]",ObjectNameEx(ProductInfo[PlayerInfo[playerid][pProductNumber_]][p_dObjectID]), ProductInfo[PlayerInfo[playerid][pProductNumber_]][p_dNumber]);
			    SendMessage(playerid, string);
			...
			else if(PlayerInfo[playerid][pProductEdit] == 7)//가격변경
			{
			    new price = strval(inputtext);
				if(0 < price <= 500)
				{
				    new string[256];
				    format(string, sizeof(string), "상품 가격이 %d$로 변경 되었습니다. [이름 : %d] [가격 : %d$]",ObjectNameEx(ProductInfo[PlayerInfo[playerid][pProductNumber_]][p_dObjectID]),price);
				    SendMessage(playerid, string);
				...
				else
				{
			        ...
			        format(string, sizeof(string), "\t"#C_GRAY"- %s 가격변경 -\n"#C_WHITE"원하시는 상품의 가격을 설정해주세요.");
			        ShowPlayerDialog(playerid, DIALOG_PRODUCT, DIALOG_STYLE_INPUT, "가격변경", string, "확인", "이전");
			        SendErrorMessage(playerid, "가격은 1~500$까지 설정 가능합니다.");
				}
			}
			return 1;
	    }
	    else
	    {
	        ...
	        if(0 < PlayerInfo[playerid][pProductEdit] <= 3)
	            OnPlayerCommandText(playerid, "/상품관리");
	    ...
	    if(response)
	    {
            ...
            if(index != -1)
            {
	            if(number + PlayerInfo[playerid][pI_Number][index] > PlayerInfo[playerid][pBuyItem][2])
					return SendErrorMessage(playerid, "구매 할 수 있는 한도 개수를 넘었습니다.");
			}
			else
			{
			...
			{
	            format(string, sizeof(string), "%s를 %d개 구매 하였습니다. [총 가격 : %d$]",ItemInfo[PlayerInfo[playerid][pBuyItem][3]][_ItemName], number, price);
	            SendMessage(playerid, string);
						...
						if(PlatoonInfo[i][p_tCreate] == 1)
						{
						    if(PlatoonInfo[i][p_tCountry] == 1)
						    {
						        if(PlatoonInfo[i][p_tType] == 1)
						        {
						            ...
						            SendMessage(playerid, "임시소대에 가입되었습니다.");
									return 1;
						        }
						    }
						}
						if(i == MAX_PLATOON -1) return SendErrorMessage(playerid, "미국엔 편성된 임시소대가 존재하지 않습니다.");
	                }
	            }
	            if(PlayerInfo[playerid][pCountry] == 2)
				{
	                for(new i = 0; i < MAX_PLATOON; i ++)
	                {
						...
						if(i == MAX_PLATOON -1) return SendErrorMessage(playerid, "러시아엔 편성된 임시소대가 존재하지 않습니다.");
	                }
	            }
	        }
	        else return SendErrorMessage(playerid, "당신은 이미 소대가 있습니다.");
	    }
		else
		{
		}
	...
	if(dialogid == DIALOG_VEHICLE_STARTUP)
	{
	    ...
	    if(response)
	    {
	        if(PlayerInfo[playerid][pVehicleStart_Cre] == 0)
	        {
	            new modelid = PlayerInfo[playerid][pVehicleStart_Model];
	            format(string, sizeof(string), "소대 : %s\n계급 : %s이상",GetVehicleStartupInfo(modelid, 1),GetVehicleStartupInfo(modelid, 2));
	            ShowPlayerDialog(playerid, DIALOG_VEHICLE_STARTUP, DIALOG_STYLE_LIST, "시동관리", string, "확인", "이전");
	            PlayerInfo[playerid][pVehicleStart_Cre] = 1;
				return 1;
	        }
	        else if(PlayerInfo[playerid][pVehicleStart_Cre] == 1)
	        {
	            if(listitem == 0)
	            {
	                ShowPlayerDialog(playerid, DIALOG_VEHICLE_STARTUP, DIALOG_STYLE_LIST, "시동관리", "추가\n삭제", "확인", "이전");
	            ...
	            if(listitem == 0)
	            {
	                ...
	                for(new i = 0; i < MAX_STARTUP_PLATOON; i ++)
	                {
						if(!VehiclestartupInfo[modelid][v_sCreate])
						{
							...
							ShowPlayerDialog(playerid, DIALOG_VEHICLE_STARTUP, DIALOG_STYLE_LIST, "시동권한 추가", str, "확인", "이전");
							PlayerInfo[playerid][pVehicleStart_Cre] = 4;
						    break;
						}
						else if(i == MAX_STARTUP_PLATOON -1) return SendErrorMessage(playerid, "더 이상 이 차량의 소대 시동 권한을 추가 할 수 없습니다.");
	                }
	            }
	            if(listitem == 1)
	            {
			...
			else if(PlayerInfo[playerid][pVehicleStart_Cre] == 4)
			{
			    ...
			    else
			    {
			        ...
			        if(CreateVehicleStartup(modelid, PlatoonID, -1) == 0) return SendErrorMessage(playerid, "더 이상 이 차량의 소대 시동 권한을 추가 할 수 없습니다.");
			        format(string, sizeof(string), "어드민 %s님이 %d번 차량의 시동권한을 %s소대에게 임명하였습니다.",PlayerInfo[playerid][pAdminName], modelid, PlatoonInfo[PlatoonID][p_tName]);
			        SendAdminMessage(string);
			    }
			    return 1;
			}
	    ...
	    {
	        if(PlayerInfo[playerid][pVehicleStart_Cre] == 0) return SendMessage(playerid, "시동 권한 추가를 취소하셨습니다.");
	        else if(PlayerInfo[playerid][pVehicleStart_Cre] == 1)
	        {
	            PlayerInfo[playerid][pVehicleStart_Cre] = 0;
				format(string, sizeof(string), "\t\t"#C_GRAY"- 시동 권한 - \n\n"#C_WHITE"%s차량의 시동 권한이 없는 상태입니다.\n'추가'버튼을 눌러 시동 권한을 추가해보세요.");
...
public ServerON()
{
	...
	else if(ServerONCheck == 2)
	{
		...
		if(LoadEnterCheck >= 0)
		    printf("[INFO] %d개의 입장/퇴장 픽업을 불러왔습니다.", LoadEnterCheck);
		if(LoadGangzoneCheck >= 0)
			printf("[INFO] %d개의 갱존을 불러왔습니다.",LoadGangzoneCheck);
		if(OccupyAreaCheck >= 0)
		    printf("[INFO] %d개의 점령지를 불러왔습니다.",OccupyAreaCheck);
		if(PlatoonCheck >= 0)
		    printf("[INFO] %d개의 소대를 불러왔습니다.",PlatoonCheck);
		if(CountryVehicleCheck >= 0)
			printf("[INFO] %d개의 국가 차량을 불러왔습니다.",CountryVehicleCheck);
		if(LoadProductCheck >= 0)
		    printf("[INFO] %d개의 상품을 불러왔습니다.",LoadProductCheck);
	}
	return 1;
}
...
public OnPlayerLogin(playerid)
{
	...
	if(PlayerInfo[playerid][pIDBan] == 1)
	{
	    ...
	    if(PlayerInfo[playerid][pIDBan] == 1) strmid(IDBanType, "아이디밴", 0, 32, 32);
	    format(string, sizeof(string), ""#C_GRAY"\t- %s - \n\n"#C_WHITE"이름 : %s\n날짜 : %d년 %d월 %d일\n담당 관리자 : %s\n이유 : %s\n\n",
		IDBanType,
	    ...
	    ShowPlayerDialog(playerid, DIALOG_NONE, DIALOG_STYLE_MSGBOX, "아이디밴 확인", string, "확인","");
	    KickEx(playerid, 1);
	}
	for(new i = 0; i < MAX_PLAYERS; i ++)
	    if(IsPlayerConnected(i))
	...
	for(new playerid = 0; playerid < MAX_PLAYERS; playerid ++)
	{
	    ...
	    if(PlayerInfo[playerid][pNPCQwestTimer] > 0)
	    {
	        ...
	        if(PlayerInfo[playerid][pNPCQwestTimer] <= 0)
	        {
	            ...
	            SendMessage(playerid, "30초간 전투연습을 했습니다. 맨 처음 픽업으로 이동해주세요.");
	            PlayerInfo[playerid][pNPCQwest] = 6;
	            ResetPlayerWeaponsEx(playerid);
	        }
	    }
...
public TutorialEnd(playerid, type)
{
	if(IsPlayerConnected(playerid) || IsPlayerLogin(playerid))
	{
		...
		else if(type == 2)
		{
		    ...
		    SetTimerEx("FadeIn", FADE_DELAY, false, "id",playerid,255);
		    TogglePlayerControllable(playerid, 1);
			format(string, sizeof(string), "%s본기지에 도착 하였습니다.",CountryNameEx(PlayerInfo[playerid][pCountry]));
			SendMessage(playerid, string);
...
public DropPlayerWeapon(playerid, DropItemID, Float:x, Float:y, Float:z, Float:angle)
{
	if(IsPlayerConnected(playerid) && IsPlayerLogin(playerid))
	{
	    ...
	    new string[128];
	    format(string, sizeof(string), "/me %s를 버린다.",DropItemInfo[DropItemID][iName]);
	    OnPlayerCommandText(playerid, string);
	}
	return 1;
}
	...
	if(IsPlayerConnected(playerid) && IsPlayerLogin(playerid))
	{
	    ...
	    if(!IsPlayerCloseWeapon(weaponid))
        {
			...
			if(animindex == 1167 || animindex == 1365 || animindex == 1161 || animindex == 1162 || animindex == 1163 || animindex == 1160)
			{
			    ...
			    {
			        new string[256];
			        format(string, sizeof(string), "%s님이 총알 무한핵으로 의심되 자동킥 당하셨습니다.", PlayerNameEx(playerid));
			        SendAdminMessage(string);
					SendMessage(playerid, "총알핵이 감지되어 자동킥 되었습니다.");
					KickEx(playerid, 1);
					return 1;
				}
				GetPlayerWeaponData(playerid, slot, PlayerInfo[playerid][pWeaponID][slot], PlayerInfo[playerid][pAmmo][slot]);
...
public C4Emplace(playerid)
{
	if(IsPlayerConnected(playerid) && IsPlayerLogin(playerid))
	{
		...
		if(PlayerInfo[playerid][pDoingGage] >= 50)
		{
			...
			OnPlayerCommandText(playerid, "/me C4를 설치한다.");
...
public PayDay(playerid)
{
    ...
    format(string, sizeof(string), "월급 : %d$", PlayerInfo[playerid][pClass]); SendMessage(playerid, string, 0x247B8AFF);
    format(string, sizeof(string), "국가지원 : %d$", 1); SendMessage(playerid, string, 0x247B8AFF);
    format(string, sizeof(string), "총 손익 : %d$", PlayerInfo[playerid][pClass] + 1); SendMessage(playerid, string, 0x247B8AFF);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    {
        new vehicleid = GetPlayerVehicleID(playerid);
        if(VehicleInfo[vehicleid][vEngine] == false) SendMessage(playerid, "이 차량은 시동이 걸려있지 않습니다.");
		else if(VehicleInfo[vehicleid][vEngine] == true) SendMessage(playerid, "이 차량은 시동이 걸려있습니다.");
...
{
	//PlayerInfo[playerid][pOldVehicle] = vehicleid;
	//SendMessage(playerid, "차에 탑승");
    return 1;
}
public OnPlayerExitVehicle(playerid, vehicleid)
{
	//PlayerInfo[playerid][pOldVehicle] = vehicleid;
	//SendMessage(playerid, "차에서 내림");
    return 1;
}
stock AntiHackSens(playerid, type)
{
	...
	if(type == 1) //체력핵
	{
		format(string, sizeof(string), "%s님이 체력핵을 시도하였습니다.", PlayerNameEx(playerid));
	}
	else if(type == 2) //아머핵
	{
	    format(string, sizeof(string), "%s님이 아머핵을 시도하였습니다.", PlayerNameEx(playerid));
	}
	else if(type == 3) //돈핵
	{
	    format(string, sizeof(string), "%s님이 돈핵을 시도하였습니다.", PlayerNameEx(playerid));
	}
	else if(type == 4)
	{
	    ...
	    format(string, sizeof(string), "%s님이 차량 수리핵을 시도하였습니다. [차량번호 : %d]", PlayerNameEx(player), player);
	}
	SendAdminMessage(string, COLOR_WARNING);
	return 1;
}
...
stock My_Information(playerid, giveplayerid)//playerid = 보여줄사람, giveplayerid = 보는 사람
{
	new string[1024];
	format(string, sizeof(string), "\t\t\t"#C_GRAY"- Infomation -\n\n"#C_WHITE"[Name] : %s\n\n[국가] : %s [성별] : %s [나이] : %d [달러] : %d [계급] : %s\n[병과] : %s [소대] : %dst [소대계급] : %s [훈장] : %d개\n[칭찬] : %d [경고] : %d/10 [시간] : %d [분] : %d [초] : %d",
	...
	ShowPlayerDialog(giveplayerid, DIALOG_NONE, DIALOG_STYLE_MSGBOX, "군번줄", string, "확인","");
	return 1;
}
...
stock CorpsName(Corps)
{
	...
	switch(Corps)
	{
	    case 0 : strmid(string, "미선택", 0, 52, 52);
		case 1 : strmid(string, "전투병", 0, 52, 52);
		case 2 : strmid(string, "공병", 0, 52, 52);
		case 3 : strmid(string, "의무병", 0, 52, 52);
		case 4 : strmid(string, "스나이퍼병", 0, 52, 52);
	}
	return string;
}
stock SexNameEx(sex)
{
    ...
    switch(sex)
	{
	    case 0 : strmid(string, "중성", 0, 12, 12);
		case 1 : strmid(string, "남성", 0, 12, 12);
		case 2 : strmid(string, "여성", 0, 12, 12);
	}
	return string;
}
stock CountryNameEx(country)
{
    ...
    switch(country)
	{
	    case 0 : strmid(string, "없음", 0, 12, 12);
		case 1 : strmid(string, "미국", 0, 12, 12);
		case 2 : strmid(string, "러시아", 0, 12, 12);
	}
	return string;
}
stock PlatRankName(level)
{
	...
	switch(level)
	{
		case 0 : strmid(string, "미소속", 0, 52, 52);
		case 1 : strmid(string, "소대원", 0, 52, 52);
		case 2 : strmid(string, "부소대장", 0, 52, 52);
		case 3 : strmid(string, "소대장", 0, 52, 52);
	}
	return string;
}
stock ArmyClass(rnak)
{
	...
	switch(rnak)
	{
		case 0 : strmid(string, "신병", 0, 52, 52);
		case 1 : strmid(string, "이병", 0, 52, 52);
		case 2 : strmid(string, "일병", 0, 52, 52);
		case 3 : strmid(string, "상병", 0, 52, 52);
		case 4 : strmid(string, "병장", 0, 52, 52);
		case 5 : strmid(string, "하사", 0, 52, 52);
		case 6 : strmid(string, "중사", 0, 52, 52);
		case 7 : strmid(string, "상사", 0, 52, 52);
		case 8 : strmid(string, "원사", 0, 52, 52);
		case 9 : strmid(string, "준위", 0, 52, 52);
		case 10 : strmid(string, "소위", 0, 52, 52);
		case 11 : strmid(string, "중위", 0, 52, 52);
		case 12 : strmid(string, "대위", 0, 52, 52);
		case 13 : strmid(string, "소령", 0, 52, 52);
		case 14 : strmid(string, "중령", 0, 52, 52);
		case 15 : strmid(string, "대령", 0, 52, 52);
		case 16 : strmid(string, "준장", 0, 52, 52);
		case 17 : strmid(string, "소장", 0, 52, 52);
		case 18 : strmid(string, "중장", 0, 52, 52);
		case 19 : strmid(string, "대장", 0, 52, 52);
	}
	return string;
}

stock EnterEntering(pickupid = 1239, Float:x,Float:y,Float:z,Float:inx,Float:iny,Float:inz,virtualworld = 0,interior = 0, Invirtualworld = -1, Ininterior = 1, type, entername[] = "무설정")
{
	for(new e = 0; e < MAX_ENTER; e ++)
	{
	    if(!EnterInfo[e][eCreate])
...
stock AutoPromotionClass(playerid)
{
	...
	{
	    new string[256];
		format(string, sizeof(string), "플레이시간이 쌓여 %s에서 %s(으)로 진급 하였습니다.",ArmyClass(PlayerInfo[playerid][pClass]-1),ArmyClass(PlayerInfo[playerid][pClass]));
		SendMessage(playerid, string);
	}
	return 1;
}
...
stock SaveOccupyArea(OccupyAreaNumber)
{
	if(Occupy_AreaInfo[OccupyAreaNumber][oCreate])
	{
		...
		mysql_format(MySQL, query, sizeof(query), "%s", query);
	    mysql_query(MySQL, query, false);
	    printf("%d번 점령지 저장",OccupyAreaNumber);
	}
	return 1;
}
	...
	for(new i = 0; i < MAX_PLAYERS; i ++)
	{
	    if(IsPlayerConnected(i) && IsPlayerLogin(i))
	    {
	        if(PlayerInfo[i][pPlatoon] == PlatoonID)
	        {
	            ...
	            SendMessage(i, "소대가 삭제되어 자동으로 탈퇴되었습니다.");
	            OnPlayerSave(i);
	        }
	    }
	}
...
stock GetPlayerJointCommand(playerid, platoonid, platoonlevel, adminlevel, type = 0)
{
	...
	if(type == 1)
		SendMessage(playerid, MessageText[10]);
	return 0;
}
    ...
    switch(num)
    {
        case 1 :
        {
			for(new i = 0; i < MAX_STARTUP_PLATOON; i ++)
			{
		    	...
		    	if(!strlen(string)) { format(string, sizeof(string), "%d소대",PlatoonInfo[VehiclestartupInfo[modelid][v_sPlatoon][i]][p_t_Number]); continue; }
				format(string, sizeof(string), "%s | %d소대",string,PlatoonInfo[VehiclestartupInfo[modelid][v_sPlatoon][i]][p_t_Number]);
				if(i == MAX_STARTUP_PLATOON - 1)
				{
				    if(!strlen(string)) strmid(string, "추가가능", 0, 128, 128);
				}
		    }
		}
		case 2 :
		{
		    if(VehiclestartupInfo[modelid][v_sClass] == -1) strmid(string, "추가가능", 0, 52, 52);
			else format(string, sizeof(string), "%s이상",ArmyClass(VehiclestartupInfo[modelid][v_sClass]));
		}
	}
	return string;
}
...
stock PlayerInfoTag(playerid, bool:show)
{
	if(show == true)
	{
	    ...
	    format(string, sizeof(string), "[계급] : %s\n[병과] : %s\n[소대] : %s", ArmyClass(PlayerInfo[playerid][pClass]),CorpsName(PlayerInfo[playerid][pCorps]), PlatoonInfo[PlayerInfo[playerid][pPlatoon]][p_tName]);
	    if(PlayerInfo[playerid][pInfoTage] == 1)
	    {
...
{
    new string[256];
    format(string, sizeof(string), "어드민 %s님에의해 당신의 계정은 정지되었습니다. [이유 : %s]",PlayerInfo[giveplayerid][pAdminName], result);
    SendMessage(playerid, string, COLOR_WARNING);
...
stock ChangePlayerCountry(playerid, country)
{
    ...
    SetPlayerPlatoonLevel(playerid, 0, 0);
    //PlayerDataCreate(playerid); // : 스폰 안 시키려면 주석 해제
    OnPlayerSpawn(playerid);// 위에 확인
	return 1;
}
...
stock CreateUserColumn()
{
	...
	print ( "[INFO] Create User Column . . . [ 30 ~ 60 Second ]" ); //느리면 최적화 된 방식으로 교체 가능.
...
stock BattalionRadio(playerid, Message[])
{
    ...
    GetPlayerPos(playerid, Pos[0], Pos[1], Pos[2]);
    format(string, sizeof(string), "[대대무전] %dst %s : %s",PlatoonInfo[platoonid][p_t_Number],PlayerNameEx(playerid, 2),Message);
    for(new i = 0; i < MAX_PLAYERS; i ++)
    {
        if(IsPlayerConnected(i) && IsPlayerLogin(i))
        {
...
stock PlatoonRadio(playerid, Message[])
{
    ...
    new Float:Pos[3], Float:gPos[3], Float:vPos[3], Float:distance = 10.0;
    GetPlayerPos(playerid, Pos[0], Pos[1], Pos[2]);
    format(string, sizeof(string), "[소대무전] (%dst)%s %s : %s",PlatoonInfo[platoonid][p_t_Number],PlayerNameEx(playerid),ArmyClass(PlayerInfo[playerid][pClass]),Message);
    for(new i = 0; i < MAX_PLAYERS; i ++)
    {
        if(IsPlayerConnected(i) && IsPlayerLogin(i))
        {
...
forward VehicleEngenStart(playerid, vehicleid);
public VehicleEngenStart(playerid, vehicleid)
{
	...
	VehicleInfo[vehicleid][vEngine] = true;
	VehicleAct(vehicleid, "시동이 걸렸습니다.");
...
stock IsPlayerVehicleStart(playerid, vehicleid)
{
	...
	{
		OnPlayerCommandText(playerid, "/me 키를 오른쪽으로 돌려 시동을 킨다.");
	...
	else if(VehicleInfo[vehicleid][vEngine] == true)
	{
		...
		VehicleInfo[vehicleid][vEngine] = false;
		OnPlayerCommandText(playerid, "/me 키를 왼쪽으로 돌려 시동을 끈다.");
		VehicleAct(vehicleid, "시동이 꺼졌습니다.");
	    return 1;
	}
	return 0;
}
//==========================포럼에서 가져온 거 수정 by.OTACON==========================//
